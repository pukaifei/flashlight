...
std::vector<double> editDist(
    std::vector<int> str1,
    std::vector<int> str2,
    bool noise_objectif = false,
    std::shared_ptr<NoiseLMLetterSwapUnit> noiselm = nullptr) {
  ...
  double dp_noise_ij_ins, dp_noise_ij_del, dp_noise_ij_sub, dp_ij_ins,
      dp_ij_del, dp_ij_sub;
  ...
  for (int i = 0; i <= m; i++) {
    for (int j = 0; j <= n; j++) {
      ...
      if (i == 0 && j == 0) {
        ...
        if (noiselm)
          dp_noise[i][j] =
              dp_noise[i][j - 1] + noiselm->scoreInsertion(str2[j - 1]);
      }
      ...
      // remove all characters of second string
        ...
        if (noiselm)
          dp_noise[i][j] =
              dp_noise[i - 1][j] + noiselm->scoreDeletion(str1[i - 1]);
      }
        ...
        dp_noise_ij_del =
            dp_noise[i - 1][j] + noiselm->scoreDeletion(str1[i - 1]);
        dp_noise_ij_sub = dp_noise[i - 1][j - 1] +
            noiselm->scoreSwap(str2[j - 1], str1[i - 1]);
...
int main(int argc, char** argv) {
  ...
  // std::make_shared<NoiseLMLetterSwapUnit>(FLAGS_probasdir,
  //                                                  FLAGS_noiselmtype,
  //                                                  noise_keys,
  //                                                  FLAGS_allowSwap,
  //                                                  FLAGS_allowInsertion,
  //                                                  FLAGS_allowDeletion,
  //                                                  false, FLAGS_scale_noise,
  //                                                  FLAGS_scale_sub,
  //                                                  FLAGS_scale_ins,
  //                                                  FLAGS_scale_del,
  //                                                  FLAGS_tkn_score);
  ...
  // auto fac_beam = ForceAlignBeamNoise(tokenDict, noiselex, *noiselm,
  // FLAGS_beamsize, FLAGS_beamthreshold, FLAGS_topk, true,
  ...
  for (auto& sample : *ds) {
    ...
    for (int b = 0; b < B; b++) {
      ...
      auto tgtsz_noisy =
          w2l::getTargetSize(tgtraw_noisy.data(), tgtraw_noisy.size());
      ...
      if (tgtraw_clean == tgtraw_noisy) {
        ...
        auto p_noise_clean =
            editDist(tgtraw_clean, tgtraw_noisy, true, noiselm_raw)[1];
        auto p_clean_clean =
            editDist(tgtraw_clean, tgtraw_clean, true, noiselm_raw)[1];
        alpha =
            (fac_noise_v[b] - fac_clean_v[b]) / (p_clean_clean - p_noise_clean);
        std::cout << "fac_noise " << fac_noise_v[b] << " fac_clean "
                  << fac_clean_v[b] << std::endl;
        std::cout << "p_clean_clean " << p_clean_clean << " p_noise_clean "
                  << p_noise_clean << std::endl;
      }
      std::cout << "alpha: " << alpha << std::endl;

      mtr_alpha.add(alpha);

      // auto edit_dist_noiseobj = editDist(tgtraw_clean, tgtraw_noisy, true,
      // noiselm_raw); auto edit_dist = editDist(tgtraw_clean, tgtraw_noisy,
      // false, noiselm_raw);

      // std::cout << "OBJ Noise, edit: " << edit_dist_noiseobj[0] << " noise: "
      // << edit_dist_noiseobj[1] << std::endl; std::cout << "     Edit, edit: "
      // << edit_dist[0] << " noise: " << edit_dist[1] << std::endl;
    }
    std::cout << "AVERAGE ALPHA " << mtr_alpha.value()[0]
              << " sd: " << sqrt(mtr_alpha.value()[1]) << std::endl;
  }
  /*
    ...
    std::endl; std::cout << "L2G clean: " << mtr_clean.value()[0] << ", standard
    deviation: " << sqrt(mtr_clean.value()[1]) << std::endl; std::cout << "L2G
    noisy: " << mtr_noisy.value()[0] << ", standard deviation: " <<
    sqrt(mtr_noisy.value()[1]) << std::endl; std::cout << "L2G clean - noisy: "
    << mtr_clean.value()[0] - mtr_noisy.value()[0] << std::endl;
  */
  return 0;
}
