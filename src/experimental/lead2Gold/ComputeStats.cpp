...
int main(int argc, char** argv) {
  ...
  std::shared_ptr<NoiseTrie> lex =
      std::shared_ptr<NoiseTrie>(new NoiseTrie(N_, -1, nullptr));
  ...
  std::cout << FLAGS_allowSwap << " " << FLAGS_allowInsertion << " "
            << FLAGS_allowDeletion << std::endl;
  // test small
  /*
  std::shared_ptr<NoiseLMLetterSwapUnit> noiselm =
  std::make_shared<NoiseLMLetterSwapUnit>(FLAGS_probasdir, "zeronoiselm",
  noise_keys, FLAGS_allowSwap, FLAGS_allowInsertion, FLAGS_allowDeletion, false,
  FLAGS_scale_noise, FLAGS_scale_ins, FLAGS_scale_del);
  std::shared_ptr<NoiseLMLetterSwapUnit> noiselm_display = noiselm;
  */

  std::shared_ptr<NoiseLMLetterSwapUnit> noiselm_display =
      std::make_shared<NoiseLMLetterSwapUnit>(
          FLAGS_probasdir,
          FLAGS_noiselmtype,
          noise_keys,
  ...
  auto fac_beam = ForceAlignBeamNoiseStats(
      noise_keys,
      lex,
      *noiselm,
      FLAGS_beamsize,
      FLAGS_beamthreshold,
      FLAGS_topk);
  auto fac_beam_normal = ForceAlignBeamNoise(
      noise_keys,
      lex,
      *noiselm,
      FLAGS_beamsize,
...
#pragma omp parallel for num_threads(nthreads)
  for (int b = 0; b < tot_B; b++) {
    ...
    auto tgtsz =
        w2l::getTargetSize(ktarget_token_v.data(), ktarget_token_v.size());
    ...
    auto tgtcleansz = w2l::getTargetSize(
        ktargetClean_token_v.data(), ktargetClean_token_v.size());
    ...
    auto fac_beam_output_normal =
        fac_beam_normal.forward(emissions, transitions, target, ktarget);
    ...
    if (FLAGS_statsDirection == "backward") {
      std::cout << "Backward normal, B = " << FLAGS_beamsize
                << " statsbeam = " << FLAGS_statsbeamsize << std::endl;
      // paths_and_weights =
      // fac_beam.extractPathsAndWeightsBackward(fac_beam_output, 0,
      // FLAGS_statsbeamsize);
      paths_and_weights = fac_beam_normal.extractPathsAndWeightsBackward(
          fac_beam_output_normal, 0, FLAGS_statsbeamsize);

      greedy_path = std::get<0>(fac_beam.greedyPath(fac_beam_output, 0)[0]);
    } else if (FLAGS_statsDirection == "backward_simple") {
      std::cout << "Backward simplify, B = " << FLAGS_beamsize
                << " statsbeam = " << FLAGS_statsbeamsize << std::endl;
      paths_and_weights = fac_beam.extractPathsAndWeightsBackwardSimplify(
          fac_beam_output, 0, FLAGS_statsbeamsize);
      greedy_path = std::get<0>(fac_beam.greedyPath(fac_beam_output, 0)[0]);
      ...
      std::cout << "Both, B = " << FLAGS_beamsize
                << " statsbeam = " << FLAGS_statsbeamsize << std::endl;
      paths_and_weights = fac_beam.extractPathsAndWeightsBoth(
          fac_beam_output, 0, FLAGS_statsbeamsize);
      greedy_path = std::get<0>(fac_beam.greedyPath(fac_beam_output, 0)[0]);
    }
    meter.stop();
    ...
    std::cout << "display path (" << paths_and_weights.size() << ")"
              << std::endl;
    ...
    for (auto& path_weight : paths_and_weights) {
      ...
      // auto trueScore = fac_beam.getTrueScore(fac_beam_output, 0,
      // proposed_path);
    ...
    std::cout << "tot score paths: " << tot_score << std::setprecision(15)
              << std::endl;
    ...
    if (!FLAGS_computeStatsStorePath.empty()) {
      ...
      std::vector<double> count_sub_M(T), count_ins_M(T), count_del_M(T),
          length_M(T), target_used_M(T);
      std::vector<double> count_sub_W(T), count_ins_W(T), count_del_W(T),
          length_W(T), target_used_W(T);
      std::vector<std::vector<std::vector<double>>> sub_token_M(T),
          sub_token_W(T);
      std::vector<std::vector<double>> del_token_M(T), del_token_W(T);
      std::vector<std::vector<double>> ins_token_M(T), ins_token_W(T);

      for (int t = 1; t <= T; t++) {
        ForceAlignBeamNoiseNodeStats aggreg_node_t =
            aggregateStatsNodes(hyps.at(t));
      ...
      if (tot_score_best_paths != -std::numeric_limits<double>::infinity()) {
        toStore += "\"tot_score_best_paths\": " +
            std::to_string(tot_score_best_paths) + ",\n";
      } else {
      ...
      for (int i = 0; i < ktargetClean_token_v.size(); i++) {
        ...
        if (i != ktargetClean_token_v.size() - 1) {
          toStore += ", ";
        }
      }
      toStore += "],\n";
      ...
      toStore += "\"tot_target_size\": " +
          std::to_string(hyps.at(1).at(0).target_size);
  ...
  if (!FLAGS_computeStatsStorePath.empty()) {
    ...
    if (outfile.is_open()) {
      ...
      for (int i = 0; i < nb_token; i++) {
        outfile << "[";
        for (int j = 0; j < nb_token; j++) {
          outfile << std::to_string(
              exp(noiselm_display->scoreSwap(i, j)) * 100);
          if (j != nb_token - 1) {
            outfile << ", ";
          }
        }
      ...
      for (int i = 0; i < nb_token; i++) {
        outfile << std::to_string(
            exp(noiselm_display->scoreInsertion(i)) * 100);
        outfile << ", ";
      }
      ...
      std::cout << "Unable to open file " << FLAGS_computeStatsStorePath
                << std::endl;
      ;
    }
  }
  ...
  // auto fac_beam_output = fac_beam.forward(emissions, transitions, target,
  // ktarget);
  ...
  //auto fac_beam_output = fac_beam.forward(emissions, transitions, target,
  ktarget); std::vector<float> fac_beam_output_host(B);
