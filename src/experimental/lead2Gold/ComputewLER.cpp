...
int main(int argc, char** argv) {
  ...
  auto fac_beam_no_noise = ForceAlignBeamNoise(
      tokenDict,
      noiselex,
      *noiselm,
      FLAGS_beamsize,
  ...
  for (auto& sample : *ds) {
    ...
    for (int b = 0; b < B; b++) {
      mtr_LER_truth.add(
          sample[kTargetIdx](af::span, b), sample[kCleanKeyIdx](af::span, b));
    }
    ...
    if (FLAGS_useevalemission) {
      fac_beam_output = fac_beam.forward(
          emission_train, emission_eval, transition, target, ktarget);

      // fac_beam_output = fac_beam.forward(emission_eval, emission_eval,
      // transition, target, ktarget); fac_beam_output =
      // fac_beam.forward(emission_eval, emission_train, transition, target,
      // ktarget);
      if (compute_no_noise) {
        fac_beam_output_no_noise = fac_beam_no_noise.forward(
            emission_train, emission_eval, transition, target, ktarget);
      }
    } else {
      fac_beam_output =
      ...
      if (compute_no_noise) {
        fac_beam_output_no_noise = fac_beam_no_noise.forward(
            emission_train, transition, target, ktarget);
      }
    }
    meter_forward.stop();
    ...
    // af::print("output end", emission(af::span, af::seq(af::end - 9,af::end),
    // 0).array());
    meter_wLER.resume();
    auto wLER_paths =
        fac_beam.wLER(fac_beam_output, ktargetClean, statsbeamsize, &mtr_wLER);
    auto& wLER_b = std::get<0>(wLER_paths);
    auto& all_paths_weights = std::get<1>(wLER_paths);
    meter_wLER.stop();

    std::cout << "batch " << sample_id << ", wLER: " << wLER_b << ", T= " << T
              << ", L= " << L << std::endl;
    // af::print("input end", input(af::seq(af::end - 9,af::end),
    // af::span,0,0).array()); af::print("output end", emission(af::span,
    // af::seq(af::end - 9,af::end), 0).array());
    ...
    for (int b = 0; b < all_paths_weights.size(); b++) {
      ...
      for (int k = 0;
           k < std::min(display_first_n, (int)paths_and_weights.size());
           k++) {
        ...
        if (compute_no_noise) {
          double scoreClean_no_noise = fac_beam_no_noise.getTrueScore(
              fac_beam_output_no_noise, b, proposed_path);
      ...
      double scoreClean =
          fac_beam.getTrueScore(fac_beam_output, b, tgtraw_clean);

      double scoreProvided = fac_beam.getTrueScore(fac_beam_output, b, tgtraw);

      if (compute_no_noise) {
        double scoreClean_no_noise = fac_beam_no_noise.getTrueScore(
            fac_beam_output_no_noise, b, tgtraw_clean);
        double scoreProvided_no_noise =
            fac_beam_no_noise.getTrueScore(fac_beam_output_no_noise, b, tgtraw);

        std::cout << "Score clean: " << scoreClean << std::endl;
        std::cout << "Score clean no noise: " << scoreClean_no_noise
                  << std::endl;
        std::cout << "Score noise clean: " << scoreClean - scoreClean_no_noise
                  << std::endl;

        auto resCleanASG = fac_asg.forward(
            emission_train(af::span, af::span, b), ktargetClean(af::span, b));
        ...
        std::cout << "Score Provided no noise: " << scoreProvided_no_noise
                  << std::endl;
        std::cout << "Score noise Provided: "
                  << scoreProvided - scoreProvided_no_noise << std::endl;

        auto resProvidedASG = fac_asg.forward(
            emission_train(af::span, af::span, b), ktarget(af::span, b));
    ...
    */
    ...
    double wLER_b = fac_beam.wLER(fac_beam_output, targetclean, statsbeamsize,
    &mtr_wLER_pad);
  ...
  std::cout << "wLER computed with " << mtr_wLER.value()[2] << " samples"
            << std::endl;
  std::cout << "wLER mean: " << mtr_wLER.value()[0] << std::endl;
  std::cout << "wLER var: " << mtr_wLER.value()[1]
            << ", wLER standard deviation: " << sqrt(mtr_wLER.value()[1])
            << std::endl;
  ...
  std::cout << "baseline LER clean/provided: " << mtr_LER_truth.value()[0]
            << ", standard deviation: " << sqrt(mtr_LER_truth.value()[1])
            << std::endl;
  // std::cout << "pad wLER computed with " << mtr_wLER_pad.value()[2] << "
  // samples" << std::endl; std::cout << "pad wLER mean: " <<
  // mtr_wLER_pad.value()[0] << std::endl; std::cout << "pad wLER var: " <<
  // mtr_wLER_pad.value()[1] << ", wLER standard deviation: "<<
  // sqrt(mtr_wLER.value()[1]) << std::endl;
