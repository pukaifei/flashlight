...
std::vector<af::array> getUpdateTrancripts(
  ...
  // auto viterbipaths_af = w2l::viterbiPath(emissions.array(),
  // transitions.array());
  ...
  for (int b = 0; b < batchsz; b++) {
    std::vector<int> viterbipath(
        viterbipaths.begin() + b * maxsizeViterbi,
        viterbipaths.begin() + (b + 1) * maxsizeViterbi);
    uniq(viterbipath);
  ...
  for (int b = 0; b < batchsz; b++) {
    std::copy(
        new_transcriptions[b].begin(),
        new_transcriptions[b].begin() + new_transcriptions[b].size(),
        new_transcriptions_flat.begin() + b * maxsize);
  }

  auto new_transcriptions_flat_unpack = unpackReplabels(
      new_transcriptions_flat, dicts[kTargetIdx], FLAGS_replabel);

  auto new_transcriptions_af =
      af::array(maxsize, batchsz, new_transcriptions_flat.data());
  auto new_transcriptions_af_unpack =
      af::array(maxsize, batchsz, new_transcriptions_flat_unpack.data());
