...
std::unordered_map<std::string, std::string> setFlags(int argc, char** argv) {
  ...
  if (runStatus == kTrainMode) {
    ...
    wasCTCBeamNoise = (FLAGS_criterion == kCtcBeamNoiseCriterion);
    readNewFlags();
  ...
  std::unordered_map<std::string, std::string> config = {
      ...
      {"wasASGBeamNoise", std::to_string(wasASGBeamNoise)},
      {"wasCTCBeamNoise", std::to_string(wasCTCBeamNoise)}};
