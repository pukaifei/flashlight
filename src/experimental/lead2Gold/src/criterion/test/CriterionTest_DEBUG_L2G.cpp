...
TEST(CriterionTestBEAM_DEBUG, ins_del_debug) {
  ...
  std::string savePath = "/checkpoint/adufraux/saveExample/80_M20_all_f1_no_ins_no_del.bin";

  af::array emission_af, transition_af, ltrTarget_af, ltrKeyTarget_af, ltrKeyTargetClean_af;

  W2lSerializer::load(savePath, emission_af, transition_af, ltrTarget_af, ltrKeyTarget_af, ltrKeyTargetClean_af);
  ...
  std::cout << "N: " << std::to_string(N) << " L: " << std::to_string(L) << " T: " << std::to_string(T) << " B: " << std::to_string(B) << std::endl;

  w2l::Dictionary noise_keys;
  std::shared_ptr<NoiseTrie> lex = std::shared_ptr<NoiseTrie>(new NoiseTrie(N, -1, nullptr));
  ...
  //lex->insert({N-1}, 0);
  //std::shared_ptr<NoiseLMLetterSwapUnit> noiselm = std::make_shared<NoiseLMLetterSwapUnit>("", "identitynoiselm", noise_keys, true, false, false, false, 1.0);

  float scale_noise=0.3;
  float scale_sub=1;
  float scale_ins=0.1;
  float scale_del=0.1;
  std::shared_ptr<NoiseLMLetterSwapUnit> noiselm = std::make_shared<NoiseLMLetterSwapUnit>("/checkpoint/adufraux/data/librispeech/chunks/noisy_datasets/from_clean100/probas/",
                                                    "M20_f1_no_ins_no_del", noise_keys, true, false, false,
                                                      false, scale_noise, scale_sub, scale_ins, scale_del);
  long beamsize = 10;
  double threshold = 0;

  auto fac_beam = ForceAlignBeamNoiseStats(noise_keys, lex, *noiselm, beamsize, threshold);
  ...
  //for (int i = 0; i < B; i++) {
  //  std::cout << "beam: " <<  std::setprecision (15) << fal_beam_output_host[i] << std::endl;
    //ASSERT_NEAR(fal_beam_output_host[i], -2.728, 1e-3);  // - 2.728 calculated by hand
  //}
 ...
 //fini.displayMatrix("weighted",false, true);
  ...
  //for (int i = 0; i < B; i++) {
    //std::cout << "beam: " << fac_beam_output_host[i] << " dynamic: " << fac_dynamic_output_host[i] << std::endl;
  //  ASSERT_NEAR(fac_beam_output_host[i], fac_dynamic_output_host[i], 1e-4); //corresponds to 0.1% of the total probability
  //}
 ...
 //af::print("fac_dynamic_grad", fac_dynamic_grad(af::span,af::seq(445,465),0),7);
 //af::print("fac_dynamic_trans_grad", fac_dynamic_trans_grad(af::span,af::span));


  //checkZero(fac_beam_grad - fac_dynamic_grad, 1e-5);
  //checkZero(fac_beam_trans_grad - fac_dynamic_trans_grad, 1e-4); // this one is very sensitive to beamsize
...
int main(int argc, char** argv) {
  ...
  //::testing::GTEST_FLAG(filter) = "*fac_beam_type0Noise_as_fac_large_input_good_emission_prob*";
