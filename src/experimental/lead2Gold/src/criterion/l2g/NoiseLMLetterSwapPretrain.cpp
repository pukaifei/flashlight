...
NoiseLMLetterSwapUnit::NoiseLMLetterSwapUnit(
    const std::string& FLAGS_probasdir,
    const std::string& FLAGS_noiselmtype,
    ...
    double scale_del,
    double tkn_score)
    : noise_keys_(noise_keys),
      allowSwap_(allowSwap),
      allowInsertion_(allowInsertion),
      allowDeletion_(allowDeletion),
      noiselmtype_(FLAGS_noiselmtype),
      autoScale_(autoScale),
      scale_noise_(scale_noise),
      scale_sub_(scale_sub),
      scale_ins_(scale_ins),
      scale_del_(scale_del),
      tkn_score_(tkn_score) {
  ...
  if (allowInsertion) {
    logProbIns.resize(
        nb_key_ + 1,
        NEG_INF_CLOSE); // +1 is to store the proba not to insert a letter.
  }
  if (allowDeletion) {
  ...
  if (FLAGS_noiselmtype == "zeronoiselm") {
    ...
    if (allowSwap) {
      logProbSwap.resize(
          nb_key_,
          std::vector<double>(
              nb_key_, -std::numeric_limits<double>::infinity()));
      for (int64_t i = 0; i < nb_key_; i++) {
        logProbSwap[i][i] = 0.0;
      }
    }
    ...
    std::string base_filename =
        std::string(FLAGS_probasdir) + std::string(FLAGS_noiselmtype);
    std::string filename;
    double proba;
      ...
      while (std::getline(f, line)) { // for every line
        ...
        while (next != end) { // for every column
          ...
          if (proba <= 0.) {
            ...
            // if (allowDeletion){
            //  logProbSwap[i][j] = std::log(proba * ((float)nb_key_ /
            //  ((float)nb_key_ + 1.)));
            //} else{
        ...
        while (next != end) { // for every column
          if (j >= nb_key_ + 1) {
            throw std::invalid_argument(
                "too much columns in insertion proba file");
          }
          std::smatch match = *next;
          proba = std::stod(match.str());
          if (proba == 0.) {
            ...
            // if (j != nb_key_){
            //  logProbIns[j] = std::log(proba * ((float)nb_key_ /
            //  ((float)nb_key_ + 1.)));
            //} else {
    ...
    if (allowDeletion) {
      ...
      while (std::getline(f, line)) { // for every line
        ...
        while (next != end) { // for every column
          if (j >= nb_key_) {
            throw std::invalid_argument(
                "too much columns in deletion proba file");
          }
          std::smatch match = *next;
          proba = std::stod(match.str());
          if (proba == 0.) {
    ...
    if (allowDeletion) {
      for (int j = 0; j < nb_key_; j++) {
        // double log_norm_value = std::log(1 + std::exp(logProbDel[j]) *
        // (scale_del_ - 1));
        ...
        for (int i = 0; i < nb_key_; i++) {
          if (i == j) {
            ...
            logProbSwap[i][j] =
                std::log(scale_sub_) + logProbSwap[i][j] - log_norm_value;
          }
        }
      ...
      for (int j = 0; j < nb_key_; j++) {
        double log_norm_value = std::log(
            std::exp(logProbSwap[j][j]) * (1 - scale_sub_) + scale_sub_);
    ...
    if (allowInsertion) {
      double log_norm_value =
          std::log(1 + std::exp(logProbIns[nb_key_]) * (1. / scale_ins_ - 1));
      for (int i = 0; i < nb_key_; i++) {
        logProbIns[i] -= log_norm_value;
      }
      logProbIns[nb_key_] =
          std::log(1. / scale_ins_) + logProbIns[nb_key_] - log_norm_value;
    }
  }
  rescale();
  // Add to params
  ...
  for (int i = 0; i < nb_key_; i++) {
    std::copy(
        logProbSwap[i].begin(),
        logProbSwap[i].begin() + nb_key_,
        logProbSwap_flat.begin() + i * nb_key_);
  }

  params_ = {fl::Variable(af::array(1, &scale_noise_), true),
             fl::Variable(
                 af::array(nb_key_ * nb_key_, logProbSwap_flat.data()), false)};
  if (allowInsertion) {
    params_.push_back(
        fl::Variable(af::array(nb_key_ + 1, logProbIns.data()), false));
  } else {
    params_.push_back(fl::Variable());
  }
  if (allowDeletion) {
    params_.push_back(
        fl::Variable(af::array(nb_key_, logProbDel.data()), false));
  } else {
    params_.push_back(fl::Variable());
  }
}
...
double NoiseLMLetterSwapUnit::scoreSwap(
    const std::string& key_noise,
    const std::string& key_clean) {
  if (allowSwap_) {
    if (!noise_keys_.contains(key_noise) || !noise_keys_.contains(key_clean)) {
      std::cout
          << "Error NoiseLMLetterSwapUnit::score, key_noise or key_clean is not a key"
          << std::endl;
    }
    auto id_noise = noise_keys_.getIndex(key_noise);
    auto id_clean = noise_keys_.getIndex(key_clean);
    // if (allowDeletion_){
    //  return scale_noise_ * (logProbSwap[id_noise][id_clean] + std::log(1 -
    //  std::exp(logProbDel[id_clean])));
    //} else{
...
double NoiseLMLetterSwapUnit::scoreInsertion(const std::string& key_noise) {
  if (allowInsertion_) {
    if (!noise_keys_.contains(key_noise)) {
      std::cout
          << "Error NoiseLMLetterSwapUnit::scoreInsertion, key_noise is not a key"
          << std::endl;
    }
    auto id_noise = noise_keys_.getIndex(key_noise);
...
double NoiseLMLetterSwapUnit::scoreDeletion(const std::string& key_clean) {
  if (allowDeletion_) {
    if (!noise_keys_.contains(key_clean)) {
      std::cout
          << "Error NoiseLMLetterSwapUnit::scoreDeletion, key_clean is not a key"
          << std::endl;
    }
    auto id_clean = noise_keys_.getIndex(key_clean);
...
double NoiseLMLetterSwapUnit::scoreSwap(int64_t id_noise, int64_t id_clean) {
  // std::cout << "swap " << id_noise << " " << noise_keys_.getEntry(id_noise)
  // << " " << id_clean << " " << noise_keys_.getEntry(id_clean) << " " <<
  // std::exp(logProbSwap[id_noise][id_clean]) << std::endl; std::cout << "scale
  // " << scale_subs_ << std::endl;
  if (allowSwap_) {
    // std::cout << "swap " << scale_noise_ * logProbSwap[id_noise][id_clean] <<
    // std::endl; if (allowDeletion_){
    //  return scale_noise_ * (logProbSwap[id_noise][id_clean] + std::log(1 -
    //  std::exp(logProbDel[id_clean])));
    //} else {
    return logProbSwap[id_noise][id_clean];
...
void NoiseLMLetterSwapUnit::trainModel(
  ...
  for (auto& sample : *ds) {
    ...
    auto output_eval =
        network->forward({fl::input(sample[w2l::kInputIdx])}).front();
    auto updatedTranscripts =
        getUpdateTrancripts(output_eval, criterion, dicts);
...
void NoiseLMLetterSwapUnit::initialize() {
  ...
  if (allowInsertion_) {
    countIns_.resize(
        nb_key_ + 1, 0); // +1 is to store the proba not to insert a letter.
  }
  if (allowDeletion_) {
...
void NoiseLMLetterSwapUnit::update(
    af::array& clean,
    af::array& noisy,
    w2l::DictionaryMap& dicts,
    int replabel) {
  auto cleanV = w2l::unpackReplabels(
      w2l::afToVector<int>(clean), dicts[w2l::kTargetIdx], replabel);
  ...
  auto noisyV = w2l::unpackReplabels(
      w2l::afToVector<int>(noisy), dicts[w2l::kTargetIdx], replabel);
    ...
    for (int j = 0; j <= n; j++) {
      ...
      // and recur for remaining string
        ...
        dp[i][j].set(
            ...
            &dp[i - 1][j - 1],
            cleanV[i - 1],
            noisyV[j - 1]); // Replace tkn with the same dp[i - 1][j - 1];
  ...
  if (allowInsertion_) { // Compute not to insert. Approximate number of time we
                         // can insert by m+1
...
// Transform into fl Variable and add to params_, go to gpu, all reduce, back to
// cpu and normnalize
void NoiseLMLetterSwapUnit::finalize(bool enable_distributed) {
  ...
  if (allowSwap_) {
    ...
    for (int i = 0; i < nb_key_; i++) {
      for (int j = 0; j < nb_key_; j++) {
        ...
        // std::cout << (double)countSwap_[i][j] << " " << (double)nb_examples_
        // << " swa-> " << countSwapNorm_[i][j] << std::endl;
      }
    }
  }
  if (allowInsertion_) {
    countInsNorm_.resize(
        nb_key_ + 1, 0); // +1 is to store the proba not to insert a letter.
    for (int i = 0; i < nb_key_ + 1; i++) {
      countInsNorm_[i] = (double)countIns_[i] / (double)nb_examples_;
      // std::cout << (double)countIns_[i] << " " << (double)nb_examples_ << "
      // ins-> " << countInsNorm_[i] << std::endl;
    }
  }
  if (allowDeletion_) {
    ...
    for (int j = 0; j < nb_key_; j++) {
      ...
      // std::cout << (double)countDel_[j] << " " << (double)nb_examples_ << "
      // del-> " << (double)countDelNorm_[j] << std::endl;
    }
  }
  ...
  if (enable_distributed) {
    ...
    for (int i = 0; i < nb_key_; i++) {
      std::copy(
          ...
          countSwapNorm_[i].begin() + nb_key_,
          countSwap_flatNorm_.begin() + i * nb_key_);
    }

    temp_params_ = {fl::Variable(
        af::array(nb_key_ * nb_key_, countSwap_flatNorm_.data()), false)};
    if (allowInsertion_) {
      temp_params_.push_back(
          fl::Variable(af::array(nb_key_ + 1, countInsNorm_.data()), false));
    }
    if (allowDeletion_) {
      temp_params_.push_back(
          fl::Variable(af::array(nb_key_, countDelNorm_.data()), false));
    }
  ...
  for (int j = 0; j < nb_key_; j++) {
    ...
    for (int i = 0; i < nb_key_; i++) {
      // std::cout << "count swap " << i << " " << j << " " << countSwap_[i][j]
      // << std::endl;
    ...
    if (allowDeletion_) {
      ...
      if (countDelNorm_[j] == 0 || tkn_count_j == 0) {
        ...
        // std::cout << "delnorm " << countDelNorm_[j] << " " << tkn_count_j <<
        // " -> " << logProbDel[j] <<  std::endl;
      }
    }
    ...
    for (int i = 0; i < nb_key_; i++) {
      if (countSwapNorm_[i][j] == 0 || tkn_count_j == 0) {
        ...
        // std::cout << "swapnorm " << countSwapNorm_[i][j] << " " <<
        // tkn_count_j << " -> " << logProbSwap[i][j] << std::endl;
      }
    }
  }
  ...
  if (allowInsertion_) {
    ...
    for (int i = 0; i < nb_key_ + 1; i++) {
      if (countInsNorm_[i] == 0 || tkn_count_ins == 0) {
        ...
        // std::cout << "insnorm " << countInsNorm_[i] << " " << tkn_count_ins
        // << " -> " << logProbIns[i]  << std::endl;
      }
    }
  }
  rescale();
...
std::vector<double> NoiseLMLetterSwapUnit::NoiseDist(
    af::array& str1_af,
    af::array& str2_af,
    w2l::DictionaryMap& dicts,
    int replabel) {
  auto str1 = w2l::unpackReplabels(
      w2l::afToVector<int>(str1_af), dicts[w2l::kTargetIdx], replabel);

  auto str1len = w2l::getTargetSize(str1.data(), str1.size());
  str1.resize(str1len);

  auto str2 = w2l::unpackReplabels(
      w2l::afToVector<int>(str2_af), dicts[w2l::kTargetIdx], replabel);
  ...
  for (int i = 0; i <= m; i++) {
    for (int j = 0; j <= n; j++) {
      ...
      // remove all characters of second string
        ...
        // std::cout << " k";
        dp_noise_ij_sub =
            dp_noise[i - 1][j - 1] + scoreSwap(str2[j - 1], str1[i - 1]);
        // std::cout << " l";

        if (dp_noise_ij_sub > dp_noise_ij_del &&
            dp_noise_ij_sub > dp_noise_ij_ins) {
          // std::cout << " k";
...
void NoiseLMLetterSwapUnit::evalBatch(
    af::array& str1_af_batch,
    af::array& str2_af_batch,
    w2l::DictionaryMap& dicts,
    int replabel,
...
void NoiseLMLetterSwapUnit::evalModel(
  ...
  for (auto& sample : *testds) {
    ...
    evalBatch(
        sample[w2l::kTargetIdx], updatedTranscripts[1], dicts, replabel, mtr);
  }
  ntwrk->train();
}
