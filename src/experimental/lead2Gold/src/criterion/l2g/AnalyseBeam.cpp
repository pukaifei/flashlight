...
// removed LM& lm,
AnalyseBeam::AnalyseBeam(
    NoiseTrie& keytrie,
    NoiseLMLetterSwapUnit& noiselm,
    long B,
    w2l::Dictionary& dict,
    double threshold)
    : fal_(keytrie, noiselm, B, threshold),
      LER(),
      tot_wLER(),
      dict_(dict),
      LER_baseline(),
...
std::vector<fl::Variable> AnalyseBeam::forward(
  ...
  auto fal_output_beam =
      fal_.forward(emissions, transitions, noisy_target, knoisy_target);
...
#pragma omp parallel for num_threads(B)
    for (int64_t b = 0; b < B;
         b++) { // look at each path for each example in the batch
    ...
    for (int64_t b = 0; b < B;
      ...
      std::cout << std::endl
                << "LER noisy / clean " << LER_baseline.value()[0] << std::endl;
      std::cout << "beam  " << fal_output_beam_host[b] << " sum " << sum
                << std::endl;
      ...
      for (const auto& p_v : pathsToValue) {
        ...
        std::reverse(
            path.begin(), path.end()); // reverse the order of the elements
        ...
        std::cout << std::endl;
        std::cout << LER.value()[0] << "  " << probas_v[i] << std::endl
                  << std::endl;
  ...
  // std::cout << " tot LER baseline " << tot_LER_baseline.value()[0] <<
  // std::endl;
  ...
  auto grad_func = [this]() { this->backward(); };
