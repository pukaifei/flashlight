 ...
 *
 */
class TransformerBlockMultiAttend : public fl::Container {
 public:
  TransformerBlockMultiAttend(
      int32_t modelDimIn1,
      int32_t modelDimIn2,
      int32_t modelDimOut,
      int32_t mlpDim,
  ...
  std::vector<fl::Variable> forward(const std::vector<fl::Variable>& input, const std::vector<fl::Variable>& encodedMulti);
 ...
 private:
  ...
  double pLayerdrop_;
  bool usePosEmb_;
  std::shared_ptr<fl::Linear> w1_, w2_, wq_auto_, wk_auto_, wv_auto_, wf_auto_, wq1_, wk1_, wv1_, wf1_, wq2_, wk2_, wv2_, wf2_;
  ...
  fl::Variable Attention(const fl::Variable& input, const fl::Variable& encoded, const int32_t offset, const int32_t numAttend);
...
CEREAL_REGISTER_TYPE(w2l::TransformerBlockMultiAttend);