...
struct CtcForceAlignBeamNoiseNode {
  ...
  CtcForceAlignBeamNoiseNode(
      long tok_,
      double score_,
      double score_forsort_,
      double tot_logProbNoise_,
      int noisytarget_t_,
      CtcForceAlignBeamNoiseNode* parent_,
      NoiseTrieNode* lexNode_,
      long lm_)
      : tok(tok_),
        score(score_),
        score_forsort(score_forsort_),
        logProbNoise(tot_logProbNoise_),
        maxscore(0.0),
...
struct CtcForceAlignBeamNoiseBatchData // could use a vector only using
...
struct CtcForceAlignBeamNoiseVariablePayload : public fl::VariablePayload {
  CtcForceAlignBeamNoiseVariablePayload(
      std::shared_ptr<CtcForceAlignBeamNoiseBatchData> data_)
      : data(std::move(data_)){};
...
class CtcForceAlignBeamNoise {
 ...
 public:
  ...
  CtcForceAlignBeamNoise(
      ...
      double threshold = 0,
      int top_k = 0,
      bool count_noise = true,
      bool count_noise_sort = false,
      int Nb_nested = 1);
  ...
  fl::Variable forward(
      ...
      fl::Variable& target);
  // fl::Variable transformOutput(fl::Variable& beam_previous, fl::Variable&
  // emissions_previous, fl::Variable& emissions_new, fl::Variable& transitions,
  // fl::Variable& target, fl::Variable& wtarget);
  static af::array viterbi(const fl::Variable& output);
  // static af::array viterbiWord(const fl::Variable& output);
  // static void showTarget(const fl::Variable& output, int64_t b, std::ostream&
  // f); // debug tool std::map< std::vector<int>, double>
  // extractPathsAndWeights(const fl::Variable& output, int64_t b); std::vector<
  // std::tuple<std::vector<int>, double>> extractPathsAndWeightsBackward(const
  // fl::Variable& output, int b, int beam_size); double getTrueScore(const
  // fl::Variable& output, int b, std::vector<int> pathAnalysis);
  // std::tuple<double, std::vector<std::vector< std::tuple<std::vector<int>,
  // double>>> > wLER(const fl::Variable& output, fl::Variable& cleantarget, int
  // beam_size, fl::AverageValueMeter* mtr_wLER = NULL);
  NoiseLMLetterSwapUnit& getNoiselm() {
    return noiselm_;
  };
  // void clearStats();
