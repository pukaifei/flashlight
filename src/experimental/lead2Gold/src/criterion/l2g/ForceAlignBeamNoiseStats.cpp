...
static void printhypsfast(
    long t,
    std::vector<ForceAlignBeamNoiseNodeStats>& hyps,
    long nhyp) {
  std::cout
      << "FST ==============================================================="
      << std::endl;
  ...
  for (long h = 0; h < nhyp; h++) {
    ...
    std::cout << "[" << h << "] t=" << t << " l=" << node.l
              << " score=" << node.score
              << " noisytarget_t=" << node.noisytarget_t << std::endl;
    norm += node.score * node.score;
  }
  std::cout << "norm=" << norm << std::endl;
}

ForceAlignBeamNoiseStats::ForceAlignBeamNoiseStats(
...
fl::Variable ForceAlignBeamNoiseStats::forward(
  ...
  // std::vector<ForceAlignBeamNoiseNodeStats> stats_node =
  // {ForceAlignBeamNoiseNodeStats()}; std::vector<ForceAlignBeamNoiseNode>
  // true_node = stats_node;
  // ForceAlignBeamNoiseBatchData<ForceAlignBeamNoiseNode> data =
  // ForceAlignBeamNoiseBatchData<ForceAlignBeamNoiseNodeStats>(); pass thre
  // correct class in the template if (compute_stats) {
  // data =
  // std::make_shared<ForceAlignBeamNoiseBatchData<ForceAlignBeamNoiseNodeStats>>();
  //} else{
  //  data =
  //  std::make_shared<ForceAlignBeamNoiseBatchData<ForceAlignBeamNoiseNode>>();
  //}
...
#pragma omp parallel for num_threads(B)
  for (int b = 0; b < B; b++) {
    ...
    for (long t = 0; t < T; t++) {
      ...
      for (ForceAlignBeamNoiseNodeStats& prev : hyps.at(t)) {
        ...
        if (top_k < N && prev.l >= 0 &&
            (std::find(
                 idx_sorted.begin(), idx_sorted.begin() + top_k, prev.l) ==
             idx_sorted.begin() + top_k)) {
          idx_sorted[K] = prev.l;
          K++;
        }
        ...
        for (int idx = 0; idx < K; idx++) {
          ...
          baseScore = prev.score + emissions_p[t * N + i] +
              (prev.l >= 0 ? transitions_v[i * N + prev.l] : 0.);
          ...
          if (prev.l ==
              -1) { // generate the first hypotheses except the rep label
            if (i != 28) {
              // if (i != 2){
              if (noiselm_.scoreSwap(
                      knoisytarget_p[prev.knoisytarget_t + 1], letter->idx()) !=
                      -std::numeric_limits<double>::
                          infinity() // If the next letter can be swapped
                  || noiselm_.scoreDeletion(letter->idx()) !=
                      -std::numeric_limits<double>::
                          infinity() // if the next letter can be deleted
                  ||
                  (noiselm_.scoreInsertion(
                   ...
                   && noiselm_.scoreSwap(
                          ...
                          letter->idx()) !=
                       -std::numeric_limits<
                           double>::infinity() // and then the next one can be
                                               // swapped or deleted (useless to
                                               // verify since already done)
                   )) {
                // newhyps.push_back({i, baseScore, prev.noisytarget_t + 1,
                // prev.knoisytarget_t + 1, &prev, letter, key, noiselmstate,
                // prev.nb_subs, prev.nb_ins, prev.nb_del}); std::cout << "prev
                // -1" << std::endl; ForceAlignBeamNoiseNodeStats& newnode =
                // newhyps[idx_nhyp]; newnode.set(i, baseScore,
                // prev.noisytarget_t + 1, prev.knoisytarget_t + 1, &prev,
                // letter, key, noiselmstate, t+1); idx_nhyp++;
            ...
            if (prev.l == i) {
              ...
              if (nb_remaining_frame >=
                ...
                if (nb_remaining_frame - 1 >= nb_remaining_target ||
                    (noiselm_.scoreSwap(
                         knoisytarget_p[prev.knoisytarget_t], letter->idx()) !=
                     -std::numeric_limits<double>::
                         infinity()) // If the next letter can be swapped
                    ||
                    (noiselm_.scoreDeletion(letter->idx()) !=
                    ...
                    || (noiselm_.scoreInsertion(
                        ...
                        && noiselm_.scoreSwap(
                               ...
                               letter->idx()) !=
                            -std::numeric_limits<double>::
                                infinity() // and then the next one can be
                                           // swapped or deleted (useless to
                                           // verify since already done)
                        )) {
              ...
              if (noiselm_.allowInsertion() &&
                ...
                if (noiselm_.scoreSwap(
                  ...
                  base_noisescore = noiselm_.scoreInsertion(
                      knoisytarget_p[prev.knoisytarget_t]);
                  noisescore = noiselm_.allowSwap() ? base_noisescore +
                          noiselm_.scoreSwap(
                              knoisytarget_p[prev.knoisytarget_t + 1],
                              prev.letter->idx())
                                                    : base_noisescore;
                  ...
                  if (noisescore != -std::numeric_limits<double>::infinity()) {
                    // ForceAlignBeamNoiseNodeStats& newnode =
                    // newhyps[idx_nhyp]; newnode.set(i, baseScore + noisescore,
                    // prev.noisytarget_t + 2, prev.knoisytarget_t + 2, &prev,
                    // letter, key, noiselmstate, t+1); idx_nhyp++;
                    newhyps.emplace_back(
                        i,
                        baseScore + noisescore,
                        ...
                        noiselmstate,
                        t + 1);
                    newhyps.back().updateSub(
                        knoisytarget_p[prev.knoisytarget_t + 1],
                        prev.letter->idx());
                    newhyps.back().updateIns(
                        knoisytarget_p[prev.knoisytarget_t]);
                  }
                }
              }

              base_noisescore = noiselm_.allowInsertion()
              ...
              if (noiselm_.allowDeletion() &&
                ...
                if (noiselm_.scoreSwap(
                  ...
                  if (nb_remaining_frame >= nb_remaining_target) {
                    noisescore = base_noisescore +
                        noiselm_.scoreDeletion(prev.letter->idx());
                    // newhyps.push_back({i, baseScore + noisescore,
                    ...
                    if (noisescore !=
                      ...
                      // ForceAlignBeamNoiseNodeStats& newnode =
                      // newhyps[idx_nhyp]; newnode.set(i, baseScore +
                      // noisescore, prev.noisytarget_t, prev.knoisytarget_t,
      ...
      std::sort(
          newhyps.begin(),
          newhyps.end(),
          [](ForceAlignBeamNoiseNodeStats& a, ForceAlignBeamNoiseNodeStats& b) {
      ...
      for (long h = 1; h < nhyp; h++) {
        ...
        if ((head.l == elem.l) &&
          ...
          if (std::isnan(coeff)) {
            ...
            std::cout << "elem score " << elem.score << " head score "
                      << head_score_ini << std::endl;
          }
      ...
      //    std::cout << "l " << hyp.l << " s " << hyp.score << " M " <<
      //    hyp.maxscore << std::endl;
      //}
    }
    ...
    for (ForceAlignBeamNoiseNodeStats& prev : hyps.at(T)) {
      ...
      if (prev.knoisytarget_t + 1 == kS &&
          noiselm_.allowSwap()) { // no insertion + swap current + no insertion
                                  // after
        base_noisescore = noiselm_.allowInsertion()
            ? 2.0 * noiselm_.scoreNoInsertion()
            : 0.; // no letter is inserted
        // base_noisescore = 0.0;
        noisescore = base_noisescore +
            noiselm_.scoreSwap(
                knoisytarget_p[prev.knoisytarget_t], prev.letter->idx());
        ...
        base_noisescore = noiselm_.scoreNoInsertion() +
            noiselm_.scoreInsertion(
                knoisytarget_p[prev.knoisytarget_t]); // no letter is inserted
        // base_noisescore =
        // noiselm_.scoreInsertion(knoisytarget_p[prev.knoisytarget_t]);
        noisescore = noiselm_.allowSwap() ? base_noisescore +
                noiselm_.scoreSwap(
                    knoisytarget_p[prev.knoisytarget_t + 1], prev.letter->idx())
                                          : base_noisescore;
        ...
        merge_it = true;
          ...
          noiselm_.allowDeletion()) { // deletion of the previous letter + no
                                      // insertion after
        base_noisescore =
            noiselm_.allowInsertion() ? noiselm_.scoreNoInsertion() : 0.;
        // base_noisescore = 0.0;
        noisescore =
            base_noisescore + noiselm_.scoreDeletion(prev.letter->idx());
  ...
  return fl::Variable(
      ...
      inputs,
      grad_func,
      std::make_shared<ForceAlignBeamNoiseStatsVariablePayload>(data));
}
...
static void accnode(
    ForceAlignBeamNoiseNodeStats& node,
    ForceAlignBeamNoiseNodeStats& prev,
    float* gemissions_p,
...
static void
print_node(std::ostream& f, long t, ForceAlignBeamNoiseNodeStats& node) {
  if (t >= 0) {
...
static void
define_node(std::ostream& f, long t, ForceAlignBeamNoiseNodeStats& node) {
  print_node(f, t, node);
...
static void connect_nodes(
    std::ostream& f,
    long t1,
    ForceAlignBeamNoiseNodeStats& node1,
    long t2,
    ForceAlignBeamNoiseNodeStats& node2) {
...
void ForceAlignBeamNoiseStats::showTarget(
...
std::vector<ForceAlignBeamNoiseNodeStats*>, int > pathTuple; typedef
...
ForceAlignBeamNoiseNodeStats* node_ptr) {
    // Add the path if not present. Otherwise add the score the already stored
path. auto it_existing_path = std::find_if(pathsInfo.begin(), pathsInfo.end(),
                          [&path, &node_ptr](const pathTuple& tuple) {
                            return (std::get<0>(tuple) == path);
                              //return ((std::get<0>(tuple) == path) &&
(std::get<3>(tuple) == node_ptr->knoisytarget_t));
                          });
...
add it std::vector<ForceAlignBeamNoiseNodeStats*> single_vect{node_ptr};
      //pathTuple new_tuple = std::make_tuple(path, value, single_vect);
      pathTuple new_tuple = std::make_tuple(path, value, single_vect,
node_ptr->knoisytarget_t); pathsInfo.push_back(new_tuple); } else{
      std::get<1>(*it_existing_path) = logadd(std::get<1>(*it_existing_path),
value); // Logadd the scores std::get<2>(*it_existing_path).push_back(node_ptr);
// Keep track of the node that has generated the path.

      //std::get<3>(*it_existing_path) = logadd(std::get<3>(*it_existing_path),
node_ptr->score);
    }
  }
...
{ return std::get<1>(a) > std::get<1>(b);
  ...
  std::map<ForceAlignBeamNoiseNodeStats*, std::set<simplePathTuple> >
getPathsByNode(){
  // Rearange pathsInfo by node. i.e. return a map where the keys are the nodes
and the value is a vector of simplePathTuple.
    std::map<ForceAlignBeamNoiseNodeStats*, std::set<simplePathTuple> >
MapByNode; for (auto const& path_tuple : pathsInfo){ for (auto const& node :
std::get<2>(path_tuple)){ simplePathTuple new_tuple =
std::make_tuple(std::get<0>(path_tuple), std::get<1>(path_tuple)); auto
it_existing = MapByNode.find(node); if (it_existing == MapByNode.end()) { //if
node is not present, we add it. MapByNode[node] = {new_tuple}; } else{
...
struct pathsInfoBeamForward {
  typedef std::tuple<
      std::vector<int>,
      double,
      std::vector<std::tuple<ForceAlignBeamNoiseNodeStats*, double>>,
      double>
      pathTuple;
  ...
  void addPathValue(
    ...
    if (it_existing_path ==
      ...
      std::vector<std::tuple<ForceAlignBeamNoiseNodeStats*, double>>
          single_vect{std::make_tuple(node_ptr, value)};
      // pathTuple new_tuple = std::make_tuple(path, value, single_vect,
      // node_ptr->score + node_ptr->maxscore); pathTuple new_tuple =
      // std::make_tuple(path, value, single_vect, (node_ptr->score +
      // node_ptr->maxscore) * node_ptr->gscore);
      pathTuple new_tuple = std::make_tuple(
          path,
          value,
          single_vect,
          value + node_ptr->gscore -
              (node_ptr->score + node_ptr->cumul_maxscore));
      pathsInfo.push_back(new_tuple);
    } else {
      std::get<1>(*it_existing_path) = logadd(
          std::get<1>(*it_existing_path),
          value); // Logadd the total score of the path

      // we search if this node has already contributed to this path.

      auto it_exist_node = std::find_if(
          std::get<2>(*it_existing_path).begin(),
          std::get<2>(*it_existing_path).end(),
          [&node_ptr](const std::tuple<ForceAlignBeamNoiseNodeStats*, double>&
                          current_node_value) {
            return (std::get<0>(current_node_value) == node_ptr);
          });

      if (it_exist_node ==
          std::get<2>(*it_existing_path)
              .end()) { // if the node has never contributed to this path
        std::get<2>(*it_existing_path)
            .push_back(std::make_tuple(
                node_ptr,
                value)); // we add a new reference to this node with the value
        // std::get<3>(*it_existing_path) =
        // logadd(std::get<3>(*it_existing_path), node_ptr->score +
        // node_ptr->maxscore); std::get<3>(*it_existing_path) +=
        // (node_ptr->score + node_ptr->maxscore) * node_ptr->gscore;
        std::get<3>(*it_existing_path) = logadd(
            std::get<3>(*it_existing_path),
            value + node_ptr->gscore -
                (node_ptr->score + node_ptr->cumul_maxscore));
      } else {
        std::get<1>(*it_exist_node) = logadd(
            std::get<1>(*it_exist_node),
            value); // else we logadd the contribution of the node to this path.
      }
    }
  }
  ...
  void beamIt(int beam_size, bool with_node_score) {
    ...
    for (auto& path : pathsInfo) {
      ...
      for (auto& tuple : std::get<2>(path)) {
        double score =
            std::get<0>(tuple)->score + std::get<0>(tuple)->cumul_maxscore;
        double next_pot = std::get<0>(tuple)->gscore - score;
        std::cout << "(" << std::get<0>(tuple) << ", "
                  << std::get<0>(tuple)->gscore << ", " << score << ", "
                  << next_pot << std::get<1>(tuple) << "),";
      }
      std::cout << "]" << std::endl;
    }
  }
  ...
  std::map<ForceAlignBeamNoiseNodeStats*, std::set<simplePathTuple>>
  getPathsByNode() {
    // Rearange pathsInfo by node. i.e. return a map where the keys are the
    // nodes and the value is a vector of simplePathTuple.
    std::map<ForceAlignBeamNoiseNodeStats*, std::set<simplePathTuple>>
        MapByNode;
    for (auto const& path_tuple : pathsInfo) {
      for (auto const& node_and_value : std::get<2>(path_tuple)) {
        ...
        simplePathTuple new_tuple =
            std::make_tuple(std::get<0>(path_tuple), value);
        auto it_existing = MapByNode.find(node);
        if (it_existing ==
            MapByNode.end()) { // if node is not present, we add it.
  ...
  std::vector<simplePathTuple> getResult(
      bool reverse) { // remove node ptr and reverse paths
    std::vector<simplePathTuple> result;
    for (auto const& path_tuple : pathsInfo) {
...
ForceAlignBeamNoiseStats::extractPathsAndWeightsForward(
    const fl::Variable& output,
    int b,
    int beam_size) {
  auto payload = output.getPayload();
        ...
        if (node.merge_a >= 0) {
          ...
          for (long idx = 0; idx < n; idx++) {
            ...
            if (it_paths_tuple != paths_by_node.end()) {
              for (auto& path_tuple : it_paths_tuple->second) {
                ...
                if (new_path.size() == 0 ||
                    (new_path.back() != node.l && node.l != -1)) {
                  new_path.push_back(node.l);
                }
                pathsInfo_current.addPathValue(
                    new_path,
                    std::get<1>(path_tuple) + node.maxscore + demerged_score -
                        previous_node->score,
                    &node);
              }
            }
          ...
          if (it_paths_tuple != paths_by_node.end()) {
            for (auto& path_tuple : it_paths_tuple->second) {
              ...
              pathsInfo_current.addPathValue(
                  new_path,
                  std::get<1>(path_tuple) + node.maxscore + node.score -
                      previous_node->score,
                  &node);
            }
          }
...
std::vector< std::tuple<std::vector<int>, double>>
ForceAlignBeamNoiseStats::extractPathsAndWeightsForward(const fl::Variable&
output, int b, int beam_size)
{
  auto payload = output.getPayload();
  if(!payload) {
    throw std::invalid_argument("expecting a payload on provided Variable");
  ...
  auto data =
std::dynamic_pointer_cast<ForceAlignBeamNoiseStatsVariablePayload>(payload)->data;
  ...
  for (int t = 1; t<=T+1; t++){
    //std::cout << t << "--------------> " <<
pathsInfo_previous.pathsInfo.size() << std::endl; pathsInfo_previous =
pathsInfo_current; pathsInfo_current = pathsInfoBeamForward(); auto
paths_by_node = pathsInfo_previous.getPathsByNode(); auto& hyps_t = hyps.at(t);
    for (auto& node : hyps_t){
      if (node.active == true){
        if (node.merge_a >= 0){
          int n = node.merge_b - node.merge_a + 1;
          for(long idx = 0; idx < n; idx++) {
            ...
            if (it_paths_tuple != paths_by_node.end()){
              for (auto& path_tuple : it_paths_tuple->second){
                auto new_path = std::get<0>(path_tuple);
                if (new_path.size() == 0 || (new_path.back() != node.l && node.l
!= -1)){ new_path.push_back(node.l);
                }
                pathsInfo_current.addPathValue(new_path, std::get<1>(path_tuple)
+ node.maxscore
                                                        + demerged_score -
previous_node->score,  &node);
              }
            }
          }
        } else{
...
struct pathsInfoBeamBackward {
  typedef std::tuple<
      std::vector<int>,
      double,
      std::map<ForceAlignBeamNoiseNodeStats*, double>,
      double>
      pathTuple;
  typedef std::tuple<std::vector<int>, double> simplePathTuple;
  std::vector<pathTuple> pathsInfo;
  ...
  void addPathValue(
    ...
    if (it_existing_path ==
        pathsInfo.end()) { // if the path is not present, we add it
      std::map<ForceAlignBeamNoiseNodeStats*, double> single_map = {
          {node_ptr, value}};
      pathTuple new_tuple =
...
std::vector<std::tuple<std::vector<int>, double>>
ForceAlignBeamNoiseStats::extractPathsAndWeightsBackward(
    const fl::Variable& output,
    int b,
    int beam_size) {
  auto payload = output.getPayload();
      ...
      for (auto const& prev_node_info : prev_nodes) {
        ...
        if (prev_node->merge_a >= 0) {
          ...
          for (long idx = 0; idx < n; idx++) {
            ...
            if (new_path.size() == 0 ||
                (new_path.back() != current_node->l && current_node->l != -1)) {
...
double ForceAlignBeamNoiseStats::getTrueScore(
    const fl::Variable& output,
    int b,
...
double ForceAlignBeamNoiseStats::wLER(
    const fl::Variable& output,
...
std::vector<std::tuple<std::vector<int>, double>>
ForceAlignBeamNoiseStats::extractPathsAndWeightsBackwardSimplify(
    const fl::Variable& output,
    int b,
    int beam_size) {
  auto payload = output.getPayload();
  ...
  for (long t = T; t >= 0; t--) {
    ...
    for (auto const& path_info : pathsInfo_previous.pathsInfo) {
      ...
      for (auto const& prev_node_info : prev_nodes) {
        auto& prev_node = prev_node_info.first;
        // std::cout << "Prev node " << prev_node  << " " << prev_node->merge_a
        // << " " << prev_node->merge_b << std::endl;
        auto& prev_path_score = prev_node_info.second;
        if (prev_node->merge_a >= 0) {
          n = prev_node->merge_b - prev_node->merge_a + 1;
...
void ForceAlignBeamNoiseStats::simplifyGraph(
    ForceAlignBeamNoiseStatsData& data_big,
    ForceAlignBeamNoiseStatsDataPtr& data_small) {
  ...
  ForceAlignBeamNoiseNodeStats* init_node_small =
      new ForceAlignBeamNoiseNodeStats(init_node_big);
  ...
  std::map<ForceAlignBeamNoiseNodeStats*, ForceAlignBeamNoiseNodeStats*>
      map_old_new = {{&init_node_big, init_node_small}};
  std::map<ForceAlignBeamNoiseNodeStats*, ForceAlignBeamNoiseNodeStats*>
      map_prev_old_new = {};

  std::cout << init_node_big.score << " " << init_node_small->score
            << std::endl;
  ...
  for (int t = 1; t <= T + 1; t++) {
    ...
    auto it_find_not_active = std::find_if(
        hyps_t_big.begin(),
        hyps_t_big.end(),
    ...
    if (it_find_not_active == hyps_t_big.end()) {
      ...
      // std::cout << "nhyp " << nhyp << " " << hyps_t_big[nhyp-1].active << " "
      // << hyps_t_big[nhyp].active << std::endl;
    }
    ...
    ForceAlignBeamNoiseNodeStats* head_small =
        new ForceAlignBeamNoiseNodeStats(head_big);
    ...
    std::cout << "1.. " << head_big.l << " " << &head_big << " " << head_small
              << std::endl;
    ...
    if (head_big.merge_a >= 0) {
      ...
      for (long idx = 0; idx < n; idx++) {
        ...
        //  std::cout << "not found 1 " << head_big.active << " " <<
        //  prev_node_big->active << " " << prev_merge_score_big << std::endl;
        //}
        score_per_merged_node[map_prev_old_new[prev_node_big]] =
            prev_merge_score_big;
      }
    } else {
      auto prev_node_big = head_big.parent;
      auto prev_merge_score_big = head_big.score;
      // if(map_prev_old_new.find(prev_node_big) == map_prev_old_new.end()){
      //    std::cout << "not found 2" << head_big.active << " " <<
      //    prev_node_big->active << " " << prev_merge_score_big << std::endl;
      //}
      score_per_merged_node[map_prev_old_new[prev_node_big]] =
          prev_merge_score_big;
    ...
    for (long h = 1; h < nhyp; h++) {
      ...
      if (head_big.l == elem_big.l) { // if we have to merge head and elem
        ...
        std::cout << "2.. " << elem_big.l << " " << &elem_big << " "
                  << head_small << std::endl;
        ...
        if (elem_big.merge_a >= 0) {
          ...
          for (long idx = 0; idx < n; idx++) {
            ...
            auto prev_merge_score_big =
                merged_score_big[idx + elem_big.merge_a];
            // if(map_prev_old_new.find(prev_node_big) ==
            // map_prev_old_new.end()){
            //  std::cout << "not found 3"  << head_big.active << " " <<
            //  prev_node_big->active << " " << prev_merge_score_big <<
            //  std::endl;
            //}
            auto it_exist_node =
                score_per_merged_node.find(map_prev_old_new[prev_node_big]);

            if (it_exist_node == score_per_merged_node.end()) {
              score_per_merged_node[map_prev_old_new[prev_node_big]] =
                  prev_merge_score_big;
            } else {
              it_exist_node->second =
                  logadd(it_exist_node->second, prev_merge_score_big);
            }
            // std::cout << "from " << prev_node_big << " " <<
            // prev_merge_score_big << std::endl;
          }
        } else {
          auto prev_node_big = elem_big.parent;
          auto prev_merge_score_big = elem_big.score;
          // if(map_prev_old_new.find(prev_node_big) == map_prev_old_new.end()){
          //  std::cout << "not found 4" << head_big.active << " " <<
          //  prev_node_big->active << " " << prev_merge_score_big << std::endl;
          //}
          auto it_exist_node =
              score_per_merged_node.find(map_prev_old_new[prev_node_big]);
        ...
        for (auto const& node_value : score_per_merged_node) {
          // std::cout << node_value.first << " " << node_value.second <<
          // std::endl;
          ...
          // std::cout << " --> " << node_value.first << " " <<
          // node_value.second << std::endl;
        }
        std::cout << "----------> " << head_small->l << " " << head_small->score
                  << std::endl;
        ...
        // std::cout << head_small->merge_a << " " << head_small->merge_b << " n
        // = " << n << std::endl; hyps_t_small.push_back(head_small);

        headidx_big = h;

        score_per_merged_node.clear();
        auto& head_big = hyps_t_big.at(headidx_big);
        // ForceAlignBeamNoiseNodeStats* head_small = new
        // ForceAlignBeamNoiseNodeStats(head_big);
        ...
        // std::cout << head_small << std::endl;
        map_old_new[&head_big] = head_small;
        std::cout << "3.. " << head_big.l << " " << &head_big << " "
                  << head_small << std::endl;
        ...
        if (head_big.merge_a >= 0) {
          ...
          for (long idx = 0; idx < n; idx++) {
            auto prev_node_big = merged_big[idx + head_big.merge_a];
            auto prev_merge_score_big =
                merged_score_big[idx + head_big.merge_a];
            // if(map_prev_old_new.find(prev_node_big) ==
            // map_prev_old_new.end()){
            //  std::cout << "not found 5" << head_big.active << " " <<
            //  prev_node_big->active << " " << prev_merge_score_big <<
            //  std::endl;
            //}
            score_per_merged_node[map_prev_old_new[prev_node_big]] =
                prev_merge_score_big;
            std::cout << prev_node_big << " " << map_prev_old_new[prev_node_big]
                      << " " << prev_merge_score_big << std::endl;
          }
        } else {
          auto prev_node_big = head_big.parent;
          auto prev_merge_score_big = head_big.score;
          // if(map_prev_old_new.find(prev_node_big) == map_prev_old_new.end()){
          //  std::cout << "not found 6" << head_big.active << " " <<
          //  prev_node_big->active << " " << prev_merge_score_big << std::endl;
          //}
          score_per_merged_node[map_prev_old_new[prev_node_big]] =
              prev_merge_score_big;
    ...
    // std::cout << "final" << &head_small << &(hyps_t_small.back()) <<
    // std::endl; std::cout << "from2 " << head_small << std::endl;
    head_small->merge_a = merged_small.size();
    head_small->score = -std::numeric_limits<double>::infinity();
    // std::cout << "NEW HEAD " << std::endl;
    for (auto const& node_value : score_per_merged_node) {
...
struct pathsInfoBeamBackward {
  typedef std::tuple<std::vector<int>, double, ForceAlignBeamNoiseNodeStats*>
pathTuple; typedef std::tuple<std::vector<int>, double> simplePathTuple;
  std::vector<pathTuple> pathsInfo;
  // This is a vector of path.
  // One path is represented by a tuple which contains:
  //  - get<0> : std::vector<int>, the actual path which is a vector a token.
  //  - get<1> : double, The associated score of this path.
  //  - get<2> : std::vector<ForceAlignBeamNoiseNodeStats*>, a vector containing
the adresses of the nodes that lead to the path.
  //             We can have multiple nodes because of the merging operation
during the forward pass and because of the multiple alignements.
...
ForceAlignBeamNoiseNodeStats* node_ptr) {
    // Add the path if not present. Otherwise add the score the already stored
path. auto it_existing_path = std::find_if(pathsInfo.begin(), pathsInfo.end(),
                          [&path, &node_ptr](const pathTuple& tuple) {return
((std::get<0>(tuple) == path) && (std::get<2>(tuple) == node_ptr));});
    // Verify if the path is present by comparing the first elemnents of the
tuples.
...
add it
      ...
      std::get<1>(*it_existing_path) = logadd(std::get<1>(*it_existing_path),
value); // Logadd the scores
      //std::get<2>(*it_existing_path).push_back(node_ptr); // Keep track of the
node that has generated the path.
    }
  }
...
{
        //return std::get<1>(a) + std::get<2>(a)->score > std::get<1>(b) +
std::get<2>(b)->score; return std::get<1>(a) + std::get<2>(a)->score >
std::get<1>(b) + std::get<2>(b)->score;
      });
  }
...
std::vector< std::tuple<std::vector<int>, double>>
ForceAlignBeamNoiseStats::extractPathsAndWeightsBackward(const fl::Variable&
output, int b, int beam_size)
{
  auto payload = output.getPayload();
  if(!payload) {
...
prev_node->maxscore;
          ...
          std::vector<int> new_path = prev_path;
          if (new_path.size() == 0 || (new_path.back() != current_node->l &&
current_node->l != -1)){ new_path.push_back(current_node->l);
          }
  ...
  for(long t = T; t >= 0; t--) {
    ...
    for (auto const& prev_node_and_path : pathsInfo_previous.getPathsByNode()){
      ...
      if(prev_node->merge_a >= 0) {
        ...
        sub_merged_gscore.resize(n);
        sub_merged_score.resize(n);
        std::copy(merged_score.begin()+prev_node->merge_a,
merged_score.begin()+prev_node->merge_b+1, sub_merged_score.begin());
...
{ current_node->gscore += sub_merged_gscore[idx];
          ...
          double score_before_merge = merged_score[idx + prev_node->merge_a] +
prev_node->maxscore;
...
current_node->l != -1)){ new_path.push_back(current_node->l);
            ...
            pathsInfo_current.addPathValue(new_path, std::get<1>(prev_path_info)
+ score_before_merge - current_node->score, current_node);
          }
          //}
        }
...
std::vector<std::tuple<std::vector<int>, double>>
ForceAlignBeamNoiseStats::extractPathsAndWeightsBoth(
    const fl::Variable& output,
    int b,
    int beam_size) {
  std::tuple<
...
#pragma omp parallel for num_threads(2)
  for (int i = 0; i < 2; i++) {
    if (i == 0) {
      std::get<0>(res_ford_back) =
          this->extractPathsAndWeightsForward(output, b, beam_size);
    } else {
      std::get<1>(res_ford_back) =
          this->extractPathsAndWeightsBackward(output, b, beam_size);
    }
  }
  ...
  for (auto& tuple_back : res_back) {
    auto it_path_merge_found = std::find_if(
        res_merge.begin(),
        res_merge.end(),
  ...
  std::sort(
      res_merge.begin(),
      res_merge.end(),
      [](std::tuple<std::vector<int>, double>& a,
         std::tuple<std::vector<int>, double>& b) {
        return std::get<1>(a) > std::get<1>(b);
      });
  ...
  std::cout << "Ford: " << tot_score_ford << ", Back: " << tot_score_back
            << ", Both: " << tot_score_merge << std::endl;

  return res_merge;
}

std::vector<std::tuple<std::vector<int>, double>>
ForceAlignBeamNoiseStats::greedyPath(const fl::Variable& output, int b) {
  auto payload = output.getPayload();
  if (!payload) {
    throw std::invalid_argument("expecting a payload on provided Variable");
  }
  ...
  w2l::uniq(path_v);
  std::vector<std::tuple<std::vector<int>, double>> result = {
      std::make_tuple(path_v, 0)};
...
current_node->score, current_node);
  ...
  for(long t = T; t >= 0; t--) {
    ...
    pathsInfo_current = pathsInfoBeamBackward();
    for (auto& path_tuple : pathsInfo_previous.pathsInfo){ // x has a
ForceAlignBeamNoiseNodeStats* key and std::vector<std::tuple<std::vector<int>,
double>> value
      ...
      if(node->merge_a >= 0) {
        long n = node->merge_b - node->merge_a + 1;
        for(long idx = 0; idx < n; idx++) {
          auto& current_node = merged[idx + node->merge_a]; // this tab gives us
directly the parent of the node auto& current_score = merged_score[idx +
node->merge_a]; if(current_score != -std::numeric_limits<double>::infinity()) {
            std::vector<int> new_path = std::get<0>(path_tuple);
            if (new_path.size() == 0 || (new_path.back() != node->l && node->l
!= -1)){ new_path.push_back(node->l);
            }
            pathsInfo_current.addPathValue(new_path, std::get<1>(path_tuple) +
current_score + node->maxscore
                                                                -
current_node->score, current_node);
          }
        }
        ...
        std::vector<int> new_path = std::get<0>(path_tuple); //x.first is the
key, the path if (new_path.size() == 0 || (new_path.back() != node->l && node->l
!= -1)){ new_path.push_back(node->l);
        }
        pathsInfo_current.addPathValue(new_path, std::get<1>(path_tuple) +
current_score + node->maxscore
...
void ForceAlignBeamNoiseStats::backward(
    std::vector<fl::Variable>& inputs,
    const fl::Variable& goutput,
...
ForceAlignBeamNoiseNodeStats aggregateStatsNodes(
    std::vector<ForceAlignBeamNoiseNodeStats>& nodes) {
...
void displayNoiseModel(
    NoiseLMLetterSwapUnit& noiselm,
    bool withInsDel,
    w2l::Dictionary* keys = nullptr) {
  int width = 6;
  int prec = 1;
  ...
  std::cout << std::setw(0) << std::setprecision(6)
            << std::defaultfloat; // get back to default display option
}
