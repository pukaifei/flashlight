...
fl::Variable TransformerBlockAttend::mlp(const fl::Variable& input) {
  float pDropout = train_ ? pDropout_ : 0.0;
  return (*w2_)(fl::dropout(fl::relu((*w1_)(input)), pDropout));
}

fl::Variable TransformerBlockAttend::getMask(int32_t n, bool cache) {
...
std::string TransformerBlockAttend::prettyString() const {
  return "TransformerBlockAttend";
}
