...
CtcForceAlignBeamNoise::CtcForceAlignBeamNoise(w2l::Dictionary& tokenDict, std::shared_ptr<NoiseTrie> lex, NoiseLMLetterSwapUnit& noiselm, long B, double threshold, int top_k, bool count_noise, bool count_noise_sort, int Nb_nested)
 : tokenDict_(tokenDict), lex_(lex), noiselm_(noiselm), B_(B), threshold_(threshold), top_k_(top_k), count_noise_(count_noise), count_noise_sort_(count_noise_sort), Nb_nested_(Nb_nested)
{
  if (Nb_nested > 1){
    omp_set_nested(true);
  } else{
...
fl::Variable CtcForceAlignBeamNoise::forward(fl::Variable& emissions, fl::Variable& noisytarget)
{
  return CtcForceAlignBeamNoise::forward(emissions, emissions, noisytarget);
}

fl::Variable CtcForceAlignBeamNoise::forward(fl::Variable& emissions, fl::Variable& emissions_forsort, fl::Variable& noisytarget)
{
...
#pragma omp parallel for num_threads(B)
  for(int b = 0; b < B; b++) {
    ...
    hyps.at(0).emplace_back(tokenDict_.getIndex("|"), 0, 0, 0, -1, nullptr, lexroot, -1);
    ...
    for(long t = 0; t < T; t++) {
      ...
      //std::vector<std::vector<CtcForceAlignBeamNoiseNode>> newhypsNested(hyps.at(t).size());
      //#pragma omp parallel for num_threads(Nb_nested_)
      for(auto& prev : hyps.at(t)) {
      //for(int prev_idx = 0;  prev_idx < hyps[t].size() ; prev_idx++) {
        //CtcForceAlignBeamNoiseNode& prev = hyps[t][prev_idx];
        //std::cout << "prev-> " << tokenDict_.getEntry(prev.tok) << " cursor: "<< tokenDict_.getEntry(noisytarget_p[prev.noisytarget_t]) << std::endl;
        //std::vector<CtcForceAlignBeamNoiseNode>& newhypsFromPrev = newhypsNested[prev_idx];
        ...
        if (lex_){
          ...
          if (std::find(idx_sorted.begin(), idx_sorted.end(), prev.tok) == idx_sorted.end()){
          ...
          if (std::find(idx_sorted.begin(), idx_sorted.end(), tokenDict_.getIndex(w2l::kBlankToken)) == idx_sorted.end()){
          ...
          K = top_k;
          if (top_k < N && prev.tok >= 0 && (std::find(idx_sorted.begin(), idx_sorted.begin() + top_k, prev.tok) == idx_sorted.begin() + top_k)){
        ...
        for( int idx = 0; idx < K; idx++ ) {
          ...
          //std::cout << tok << " <- " << tokenDict_.getEntry(tok) << std::endl;
          //std::cout << tokenDict_.getEntry(noisytarget_p[prev.noisytarget_t]) << " " << tokenDict_.getEntry(noisytarget_p[prev.noisytarget_t + 1]) << " " << tokenDict_.getEntry(noisytarget_p[prev.noisytarget_t + 2]) << std::endl;
          ...
          if (t == 0){ // generate the first hypothesis except the rep label
              if (tok == tokenDict_.getIndex(w2l::kBlankToken)
                  || std::isfinite(noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 1], tok)) // If the next letter can be swapped
                  || std::isfinite(noiselm_.scoreDeletion(tok)) //if the next letter can be deleted
                  || (std::isfinite(noiselm_.scoreInsertion(noisytarget_p[prev.noisytarget_t + 1])) && (prev.noisytarget_t + 2 < S) // if the next letter can be inserted
                        && std::isfinite(noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 2], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                       )
                  )
              {
                baseScore = prev.score + emissions_p[t*N+tok];
                baseScore_forsort = prev.score_forsort + emissions_forsort_p[t*N+tok];
                newhyps.emplace_back(tok, baseScore, baseScore_forsort, 0, prev.noisytarget_t + 1, &prev, lexNode, noiselmstate);
                //std::cout << "New hyp " << "<-- " << tokenDict_.getEntry(tok) << std::endl;
              }
          } else{
            ...
            if (prev.tok == tokenDict_.getIndex(w2l::kBlankToken)){
              ...
              if ((tok == tokenDict_.getIndex(w2l::kBlankToken)) 
               && (nb_remaining_frame >= nb_remaining_target + 1)){
                newhyps.emplace_back(tok, baseScore, baseScore_forsort, 0, prev.noisytarget_t, &prev, lexNode, noiselmstate);
                //std::cout << "New hyp " << "<-- " << tokenDict_.getEntry(tok) << std::endl;
              }
              if ((tok != tokenDict_.getIndex(w2l::kBlankToken))
                && (std::isfinite(noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t], tok))) // If the next letter can be swapped
                  || (std::isfinite(noiselm_.scoreDeletion(tok)) && (nb_remaining_frame - 1 >= nb_remaining_target)) //if the next letter can be deleted
                  || (std::isfinite(noiselm_.scoreInsertion(noisytarget_p[prev.noisytarget_t])) && (prev.noisytarget_t + 2 < S) // if the next letter can be inserted
                        && std::isfinite(noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 1], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                   )
                 ){
                newhyps.emplace_back(tok, baseScore, baseScore_forsort, 0, prev.noisytarget_t, &prev, lexNode, noiselmstate);
                //std::cout << "New hyp " << "<-- " << tokenDict_.getEntry(tok) << std::endl;
              ...
              if (nb_remaining_frame >= nb_remaining_target){ 
                ...
                {
                  newhyps.emplace_back(tok, baseScore, baseScore_forsort, 0, prev.noisytarget_t, &prev, prev.lexNode, noiselmstate);
                  //std::cout << "New hyp " << "<-- " << tokenDict_.getEntry(tok) << std::endl;
                }
              }
            } else {
              // or we change the letter, hence the key
              if (noiselm_.allowInsertion() && prev.noisytarget_t + 2 < S){ //a letter has been added to the noisy transcription. +1 only because we can 
                if (
                  (tok == tokenDict_.getIndex(w2l::kBlankToken) && nb_remaining_frame >= nb_remaining_target - 1)
                  || (tok != tokenDict_.getIndex(w2l::kBlankToken) &&
                      (std::isfinite(noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 2], tok))) // If the next letter can be swapped
                      || (std::isfinite(noiselm_.scoreDeletion(tok)) && (nb_remaining_frame >= nb_remaining_target - 1)) //if the next letter can be deleted
                      || (std::isfinite(noiselm_.scoreInsertion(noisytarget_p[prev.noisytarget_t + 2])) && (prev.noisytarget_t + 4 < S) // if the next letter can be inserted
                        && std::isfinite(noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 3], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                         )
                     )
                  )
                {
                  logProbNoise = noiselm_.scoreInsertion(noisytarget_p[prev.noisytarget_t]) + noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 1], prev.tok);
                  ...
                  if (std::isfinite(noisescore)){
                    ...
                    newhyps.emplace_back(tok, score, score_forsort, logProbNoise, prev.noisytarget_t + 2, &prev, lexNode, noiselmstate);
                    //std::cout << "New hyp " << "<-- " << tokenDict_.getEntry(tok) << std::endl;
                  }
                }
              }
              ...
              if (noiselm_.allowDeletion() && prev.noisytarget_t < S){ // for now allow only 1 deletion and one last deletion
                    ...
                    )
                {
                  ...
                  if (nb_remaining_frame >= nb_remaining_target){
                    ...
                    noisescore = noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score();           
                    //newhyps.push_back({i, baseScore + noisescore, prev.noisytarget_t, prev.noisytarget_t, &prev, letter, key, noiselmstate, prev.nb_subs, prev.nb_ins, prev.nb_del + 1});
                    if (std::isfinite(noisescore)){
                      score = count_noise_ ? baseScore + noisescore : baseScore;
                      score_forsort = count_noise_sort_ ? baseScore_forsort + noisescore : baseScore_forsort;
                      newhyps.emplace_back(tok, score, score_forsort, logProbNoise, prev.noisytarget_t, &prev, lexNode, noiselmstate);
                      //std::cout << "New hyp " << "<-- " << tokenDict_.getEntry(tok) << std::endl;
                    }
                  }
                }
              ...
              if (noiselm_.allowSwap() && prev.noisytarget_t + 1 < S){
                ...
                //std::cout << "swap " << tokenDict_.getEntry(tok)  << "remain frame " << nb_remaining_frame << " remain tar " << nb_remaining_target << std::endl;
                if (
                    (tok == tokenDict_.getIndex(w2l::kBlankToken) && nb_remaining_frame >= nb_remaining_target)
                    || (tok != tokenDict_.getIndex(w2l::kBlankToken) &&
                        std::isfinite(noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 1], tok)) // If the next letter can be swapped
                        || (std::isfinite(noiselm_.scoreDeletion(tok)) && (nb_remaining_frame - 1 >= nb_remaining_target - 1)) //if the next letter can be deleted
                        || (std::isfinite(noiselm_.scoreInsertion(noisytarget_p[prev.noisytarget_t + 1])) && (prev.noisytarget_t + 3 < S) // if the next letter can be inserted
                            && std::isfinite(noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 2], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                           )
                        )
                    )
                {
                  logProbNoise = base_logProbNoise + noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t], prev.tok);
                  noisescore = noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score();
                  if (std::isfinite(noisescore)){
                    score = count_noise_ ? baseScore + noisescore : baseScore;
                    score_forsort = count_noise_sort_ ? baseScore_forsort + noisescore : baseScore_forsort;
                    newhyps.emplace_back(tok, score, score_forsort, logProbNoise, prev.noisytarget_t + 1, &prev, lexNode, noiselmstate);
                    //std::cout << "New hyp " << "<-- " << tokenDict_.getEntry(tok) << std::endl;
                  }
                }
              }  
    ...
    //}
    //std::cout << "tot size hyps before merge " << newhyps.size() << std::endl;
    ...
    if(newhyps.size() == 0) {
      ...
      std::sort(newhyps.begin(), newhyps.end(), [](CtcForceAlignBeamNoiseNode& a, CtcForceAlignBeamNoiseNode& b) {
          if (a.lexNode == b.lexNode){
            if(a.tok == b.tok) { // same as a.lex == b.lex but count the rep1 token
      ...
      for(long h = 1; h < nhyp; h++) {
        ...
        if((head.lexNode == elem.lexNode) && (head.tok == elem.tok) && (head.noisytarget_t == elem.noisytarget_t)) { //maybe to change when key level
          if(head.merge_a < 0) {
      ...
      // beam it
      std::sort(newhyps.begin(), newhyps.begin()+nhyp, [](CtcForceAlignBeamNoiseNode& a, CtcForceAlignBeamNoiseNode& b) { return a.score_forsort > b.score_forsort; });
    ...
    for(CtcForceAlignBeamNoiseNode &prev : hyps.at(T)) {
      ...
      if (prev.tok == tokenDict_.getIndex(w2l::kBlankToken)){
        merge_it = true;
        noisescore = 0;
      } else if (prev.noisytarget_t + 1 == S && noiselm_.allowSwap()){ // no insertion + swap current + no insertion after
        base_logProbNoise = noiselm_.allowInsertion() ? 2.0 * noiselm_.scoreNoInsertion() : 0.;
        logProbNoise = base_logProbNoise + noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t], prev.tok);
        noisescore = noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score();
        merge_it = true;
      } else if (prev.noisytarget_t + 2 == S && noiselm_.allowInsertion()){ // insertion + swap current + no insertion
        base_logProbNoise = noiselm_.scoreNoInsertion() + noiselm_.scoreInsertion(noisytarget_p[prev.noisytarget_t]);
        logProbNoise = noiselm_.allowSwap() ? base_logProbNoise + noiselm_.scoreSwap(noisytarget_p[prev.noisytarget_t + 1], prev.tok) : base_logProbNoise;
        noisescore = noiselm_.allowSwap() ? noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score() : noiselm_.scale_noise() * logProbNoise;
        merge_it = true;
      } else if (prev.noisytarget_t == S && noiselm_.allowDeletion()){ //deletion of the previous letter + no insertion after
  ...
  return fl::Variable(result, inputs, grad_func, std::make_shared<CtcForceAlignBeamNoiseVariablePayload>(data));
}
...
static void accnode(CtcForceAlignBeamNoiseNode& node, CtcForceAlignBeamNoiseNode& prev, float* gemissions_p, long t, long T, long N, NoiseLMLetterSwapUnit& noiselm, double g)
{
  if(t >= 0 && t < T) {
...
void CtcForceAlignBeamNoise::backward(std::vector<fl::Variable>& inputs, const fl::Variable& goutput, std::shared_ptr<CtcForceAlignBeamNoiseBatchData> data)
{
          ...
          if(node.merge_a >= 0) {
            ...
            for(long idx = 0; idx < n; idx++) {
              if(sub_merged_score[idx] != -std::numeric_limits<double>::infinity()) {
                accnode(node, *merged.at(node.merge_a+idx), gemissions_p, t, T, N, noiselm_, sub_merged_gscore[idx]);
              }
            }
          } else {
            accnode(node, *node.parent, gemissions_p, t, T, N, noiselm_, node.gscore);
          }
        }
      }
    }
...
af::array CtcForceAlignBeamNoise::viterbi(const fl::Variable& output)
{
  ...
  auto data = std::dynamic_pointer_cast<CtcForceAlignBeamNoiseVariablePayload>(payload)->data;
...
std::tuple<double, std::vector<std::vector< std::tuple<std::vector<int>, double>>> > ForceAlignBeamNoise::wLER(const fl::Variable& output, fl::Variable& cleantarget, int beam_size, fl::AverageValueMeter* mtr_wLER)
{
  ...
  auto& data = std::dynamic_pointer_cast<CtcForceAlignBeamNoiseVariablePayload>(payload)->data->batch;
