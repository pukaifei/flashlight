...
void NoiseTrie::apply(
    NoiseTrieNode* node,
    std::function<void(NoiseTrieNode*)> func) {
  func(node);
...
void NoiseTrieNode::smearing(
  ...
  for (NoiseTrieLabel& label : labels_) {
    node_scores.at(id_) =
        std::max(node_scores.at(id_), word_scores.at(label.lm));
  }
  for (auto child : children_) {
    if (child) {
      child->smearing(word_scores, node_scores); /* it is recursive */
      node_scores.at(id_) =
          std::max(node_scores.at(id_), node_scores.at(child->id_));
    }
  }
}

void NoiseTrie::smearing(
    const std::vector<float>& word_scores,
    std::vector<float>& node_scores) {
  root_->smearing(word_scores, node_scores);
}

w2l::Dictionary NoiseTrie::load(
