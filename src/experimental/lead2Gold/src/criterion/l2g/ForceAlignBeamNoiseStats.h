...
struct ForceAlignBeamNoiseNodeStats {
  ...
  ForceAlignBeamNoiseNodeStats*
      parent; // shared pointeur // only for backtracking (Viterbi)
  ...
  //  delete length_trans_ptr, nb_target_used_ptr, sub_token_ptr, del_token_ptr,
  //  ins_token_ptr, w_length_trans_ptr, w_nb_target_used_ptr, w_sub_token_ptr,
  //  w_del_token_ptr, w_ins_token_ptr;
  //}
  ...
  ForceAlignBeamNoiseNodeStats(
      ...
      int noisytarget_t_,
      int knoisytarget_t_,
      ForceAlignBeamNoiseNodeStats* parent_,
      NoiseTrieNode* letter_,
      NoiseTrieLabel* key_,
      long lm_,
      int nb_token_,
      int target_size_,
      ...
      : l(l_),
        ...
        tot_frame(tot_frame_) {
    length_trans = 0.;
    nb_target_used = 0.;
    sub_token = std::vector<std::vector<double>>(
        nb_token, std::vector<double>(nb_token, 0.));
    del_token = std::vector<double>(nb_token, 0.);
    ins_token = std::vector<double>(nb_token, 0.);

    w_length_trans = 0.;
    w_nb_target_used = 0.;
    w_sub_token = std::vector<std::vector<double>>(
        nb_token, std::vector<double>(nb_token, 0.));
  ...
  void set(
    ...
    sub_token_ptr =
    std::make_shared<std::vector<std::vector<double>>>(*parent_->sub_token_ptr);
    del_token_ptr =
    std::make_shared<std::vector<double>>(*parent_->del_token_ptr);
    ins_token_ptr =
    std::make_shared<std::vector<double>>(*parent_->ins_token_ptr);

    w_length_trans_ptr = std::make_shared<double>(*parent_->w_length_trans_ptr);
    w_nb_target_used_ptr =
    std::make_shared<double>(*parent_->w_nb_target_used_ptr); w_sub_token_ptr =
    std::make_shared<std::vector<std::vector<double>>>(*parent_->w_sub_token_ptr);
    w_del_token_ptr =
    std::make_shared<std::vector<double>>(*parent_->w_del_token_ptr);
    w_ins_token_ptr =
    std::make_shared<std::vector<double>>(*parent_->w_ins_token_ptr);
    */
  }
  ...
  int noisytarget_t;
  int knoisytarget_t;
  ForceAlignBeamNoiseNodeStats* parent; //shared pointeur // only for
  backtracking (Viterbi) NoiseTrieNode *letter; // shared pointeur
  NoiseTrieLabel *key; // share pointeur
  long lm;
  long merge_a;
  long merge_b;
  ...
  ForceAlignBeamNoiseNodeStats()
  :l(-1), score(0), maxscore(0), noisytarget_t(-1), knoisytarget_t(-1),
  parent(nullptr), letter(nullptr), key(nullptr), lm(-1), merge_a(-1),
  merge_b(-1), active(false), sum_coeff(1.)
  {
  }

  ForceAlignBeamNoiseNodeStats(std::string mode)
    : l(-1), score(-1e200), maxscore(-1), noisytarget_t(-1), knoisytarget_t(-1),
  parent(nullptr), letter(nullptr), key(nullptr), lm(-1), merge_a(-1),
  merge_b(-1), active(false), sum_coeff(1.)
  {
  }

  ForceAlignBeamNoiseNodeStats(long l_, double score_, int noisytarget_t_, int
  knoisytarget_t_, ForceAlignBeamNoiseNodeStats *parent_, NoiseTrieNode
  *letter_, NoiseTrieLabel *key_, long lm_, int nb_token_, int target_size_, int
  current_frame_, int tot_frame_) : l(l_), score(score_), maxscore(0.0),
  noisytarget_t(noisytarget_t_), knoisytarget_t(knoisytarget_t_),
  parent(parent_), letter(letter_), key(key_), lm(lm_), merge_a(-1),
  merge_b(-1), active(false), nb_token(nb_token_), sum_coeff(1.),
  target_size(target_size_), current_frame(current_frame_),
  tot_frame(tot_frame_)
  {
    length_trans_ptr = new double(0.);
    nb_target_used_ptr = new double(0.);
    sub_token_ptr = new std::vector<std::vector<double>>(nb_token,
  std::vector<double>(nb_token, 0.)); del_token_ptr = new
  std::vector<double>(nb_token, 0.); ins_token_ptr = new
  std::vector<double>(nb_token, 0.);

    w_length_trans_ptr = new double(0.);
    w_nb_target_used_ptr = new double(0.);
    w_sub_token_ptr = new std::vector<std::vector<double>>(nb_token,
  std::vector<double>(nb_token, 0.)); w_del_token_ptr = new
  std::vector<double>(nb_token, 0.); w_ins_token_ptr = new
  std::vector<double>(nb_token, 0.);
  }

  ForceAlignBeamNoiseNodeStats(long l_, double score_, int noisytarget_t_, int
  knoisytarget_t_, ForceAlignBeamNoiseNodeStats *parent_, NoiseTrieNode
  *letter_, NoiseTrieLabel *key_, long lm_, int current_frame_) : l(l_),
  score(score_), maxscore(0.0), noisytarget_t(noisytarget_t_),
  knoisytarget_t(knoisytarget_t_), parent(parent_), letter(letter_), key(key_),
  lm(lm_), merge_a(-1), merge_b(-1), active(false), nb_token(parent_->nb_token),
  sum_coeff(1.), length_trans_ptr(parent_->length_trans_ptr),
  nb_target_used_ptr(parent_->nb_target_used_ptr),
  sub_token_ptr(parent_->sub_token_ptr), del_token_ptr(parent_->del_token_ptr),
  ins_token_ptr(parent_->ins_token_ptr),
      w_length_trans_ptr(parent_->w_length_trans_ptr),
  w_nb_target_used_ptr(parent_->w_nb_target_used_ptr),
  w_sub_token_ptr(parent_->w_sub_token_ptr),
  w_del_token_ptr(parent_->w_del_token_ptr),
  w_ins_token_ptr(parent_->w_ins_token_ptr), target_size(parent_->target_size),
  tot_frame(parent_->tot_frame), current_frame(current_frame_)
  {
  }
  ...
  void updateSub(int noisy_, int clean_){
    ...
    if (sub_token_ptr == parent->sub_token_ptr){
      sub_token_ptr = new std::vector<std::vector<double>>
  (*(parent->sub_token_ptr)); w_sub_token_ptr = new
  std::vector<std::vector<double>> (*(parent->w_sub_token_ptr));
    }
  ...
  void mergeWStats(ForceAlignBeamNoiseNodeStats& elem, double& coeff){
    ...
    if (w_sub_token_ptr == parent->w_sub_token_ptr){
      w_sub_token_ptr = new std::vector<std::vector<double>>
  (*(parent->w_sub_token_ptr));
    }
  ...
  */
  ...
  void displayStats(std::string mode = "max") {
    ...
    if (length > 0) {
      ...
      std::cout << "Nb target used: " << target_used << ", " << ratio_target
                << "%" << std::endl;
      std::cout << "Frame: " << current_frame << ", " << ratio_frame << "%"
                << std::endl;
  ...
  void displayMatrix(
    ...
    // // display option
    std::cout << std::endl
              << "SUB matrix mode " << mode << ", norm: " << norm << std::endl;
  ...
  // ForceAlignBeamNoiseNode(ForceAlignBeamNoiseNode&) = delete; // deactivate
  // copy constructor ForceAlignBeamNoiseNode&
  // operator=(ForceAlignBeamNoiseNode&&) = default;
};

ForceAlignBeamNoiseNodeStats aggregateStatsNodes(
    std::vector<ForceAlignBeamNoiseNodeStats>&);
...
struct ForceAlignBeamNoiseStatsData {
  std::vector<std::vector<ForceAlignBeamNoiseNodeStats>>
      hyps; // use share pointeur // vector of hyp. 1 hyp is a vector of
            // NoiseNode.
...
struct
    ForceAlignBeamNoiseStatsBatchData // could use a vector only using
                                      // ForceAlignBeamNoiseBatchData =
                                      // std::vector<ForceAlignBeamNoiseData>;
{
...
{
  std::vector< std::vector<ForceAlignBeamNoiseNode> > hyps; // use share
pointeur // vector of hyp. 1 hyp is a vector of NoiseNode.
  std::vector<ForceAlignBeamNoiseNode*> merged; //use share pointeur
  std::vector<double> merged_score;
};


struct ForceAlignBeamNoiseBatchData // could use a vector only using
ForceAlignBeamNoiseBatchData = std::vector<ForceAlignBeamNoiseData>;
{
...
struct ForceAlignBeamNoiseStatsVariablePayload : public fl::VariablePayload {
  ForceAlignBeamNoiseStatsVariablePayload(
      std::shared_ptr<ForceAlignBeamNoiseStatsBatchData> data_)
      : data(std::move(data_)){};
...
class ForceAlignBeamNoiseStats {
 ...
 public:
  ...
  static void showTarget(
      ...
      int64_t b,
      std::ostream& f); // debug tool
  std::vector<std::tuple<std::vector<int>, double>>
  extractPathsAndWeightsBackward(
      const fl::Variable& output,
      int b,
      int beam_size);
  double getTrueScore(
  ...
  double
  wLER(const fl::Variable& output, fl::Variable& cleantarget, int& beam_size);
  std::vector<std::tuple<std::vector<int>, double>>
  extractPathsAndWeightsBackwardSimplify(
      const fl::Variable& output,
      int b,
      int beam_size);
  std::vector<std::tuple<std::vector<int>, double>>
  extractPathsAndWeightsForward(
      const fl::Variable& output,
      int b,
      int beam_size);
  std::vector<std::tuple<std::vector<int>, double>>
  extractPathsAndWeightsBoth(const fl::Variable& output, int b, int beam_size);
  std::vector<std::tuple<std::vector<int>, double>> greedyPath(
      const fl::Variable& output,
      int b);
  void simplifyGraph(
