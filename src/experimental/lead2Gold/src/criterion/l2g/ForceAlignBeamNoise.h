...
struct ForceAlignBeamNoiseNode {
  long tokNet; // token index in the network. can differ from tok because it can
               // be the rep label.
  ...
  ForceAlignBeamNoiseNode(
      ...
      int noisytarget_t_,
      int knoisytarget_t_,
      ForceAlignBeamNoiseNode* parent_,
...
struct ForceAlignBeamNoiseData {
  std::vector<std::vector<ForceAlignBeamNoiseNode>>
...
struct ForceAlignBeamNoiseVariablePayload : public fl::VariablePayload {
  ForceAlignBeamNoiseVariablePayload(
      std::shared_ptr<ForceAlignBeamNoiseBatchData> data_)
      : data(std::move(data_)){};
...
class ForceAlignBeamNoise {
 ...
 public:
  ...
  fl::Variable forward(
      ...
      fl::Variable& target,
      fl::Variable& wtarget);
  fl::Variable transformOutput(
      fl::Variable& beam_previous,
      fl::Variable& emissions_previous,
      fl::Variable& emissions_new,
      fl::Variable& transitions,
      fl::Variable& target,
      fl::Variable& wtarget);
  ...
  // std::map< std::vector<int>, double> extractPathsAndWeights(const
  // fl::Variable& output, int64_t b);
  ...
  wLER(
      ...
      int beam_size,
      fl::AverageValueMeter* mtr_wLER = NULL);
