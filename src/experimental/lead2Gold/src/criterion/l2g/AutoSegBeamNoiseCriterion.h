...
public:
  AutoSegBeamNoiseCriterion(int N, w2l::Dictionary& tokenDict, std::shared_ptr<NoiseTrie> lex, NoiseLMLetterSwapUnit& lm, long B, w2l::CriterionScaleMode scaleMode_, double threshold=0, bool computeStats=false, int top_k=0, bool useevalemission=false, bool useNoiseToSort=false);
  std::vector<fl::Variable> forward(fl::Variable& emissions, fl::Variable& emissions_eval, fl::Variable transitions, fl::Variable target, fl::Variable wtarget, fl::Variable cleantarget = fl::Variable(), int statsbeamsize = 5, fl::AverageValueMeter* mtr_wLER = nullptr);
