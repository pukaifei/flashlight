...
/*
static void printhypsfast(long t, std::vector<ForceAlignBeamNoiseNode>& hyps, long nhyp)
{
...
*/
ForceAlignBeamNoise::ForceAlignBeamNoise(w2l::Dictionary& tokenDict, std::shared_ptr<NoiseTrie> lex, NoiseLMLetterSwapUnit& noiselm, long B, double threshold, int top_k, bool count_noise, bool count_noise_sort)
 : tokenDict_(tokenDict), lex_(lex), noiselm_(noiselm), B_(B), threshold_(threshold), top_k_(top_k), count_noise_(count_noise), count_noise_sort_(count_noise_sort)
{
}
...
fl::Variable ForceAlignBeamNoise::forward(fl::Variable& emissions, fl::Variable& transitions, fl::Variable& noisytarget, fl::Variable& knoisytarget)
{
  return ForceAlignBeamNoise::forward(emissions, emissions, transitions, noisytarget, knoisytarget);
}

fl::Variable ForceAlignBeamNoise::forward(fl::Variable& emissions, fl::Variable& emissions_forsort, fl::Variable& transitions, fl::Variable& noisytarget, fl::Variable& knoisytarget)
{
  ...
  int top_k = top_k_ <= 0 ? N : std::min(std::max(2, top_k_), N); // we don't allow top_k = 1
...
#pragma omp parallel for num_threads(B)
  for(int b = 0; b < B; b++) {
    ...
    hyps.at(0).emplace_back(tokenDict_.getIndex("|"), tokenDict_.getIndex("|"), 0, 0, 0, -1, -1, nullptr, lexroot, -1);
    ...
    for(long t = 0; t < T; t++) {
      ...
      if (!lex_ && top_k < N){ // prune top k
        idx_sorted = idx_unsort;
        std::partial_sort( idx_sorted.begin(), idx_sorted.begin() + top_k, idx_sorted.end(), Comp(emissions_forsort_p + t*N) );
        //std::partial_sort( idx_sorted.begin(), idx_sorted.begin() + top_k, idx_sorted.end(), Comp(emissions_p + t*N) );
      }
      //if (b==3){
      //  std::cout << "t: " << t << " size hyp: " << hyps.at(t).size() << std::endl;
      //}
      for(ForceAlignBeamNoiseNode& prev : hyps.at(t)) {

        //add previous token if not in top k

        //std::cout << "prev.l: " << prev.l << " prev target: " << knoisytarget_p[prev.knoisytarget_t ] << std::endl;
        if (lex_){
          idx_sorted.clear();
        ...
        } else { // add the previous token if not present if we don't use a lexicon
          K = top_k;
          if (top_k < N && prev.tokNet >= 0 && (std::find(idx_sorted.begin(), idx_sorted.begin() + top_k, prev.tokNet) == idx_sorted.begin() + top_k)){
        ...
        for( int idx = 0; idx < K; idx++ ) {
          ...
          if (lex_){
            lexNode = (tok == tokenDict_.getIndex("|")) ? lexroot : prev.lexNode->child(tok);
          }

          if (t == 0){ // generate the first hypothesis except the rep label
            if (i != tokenDict_.getIndex("1")){
              if (std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 1], tok)) // If the next letter can be swapped
                  || std::isfinite(noiselm_.scoreDeletion(tok)) //if the next letter can be deleted
                  || (std::isfinite(noiselm_.scoreInsertion(knoisytarget_p[prev.knoisytarget_t + 1])) && (prev.knoisytarget_t + 2 < kS) // if the next letter can be inserted
                        && std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 2], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                       )
                  )
              {
                baseScore = prev.score + emissions_p[t*N+i];
                baseScore_forsort = prev.score_forsort + emissions_forsort_p[t*N+i];
                newhyps.emplace_back(i, tok, baseScore, baseScore_forsort, 0, prev.noisytarget_t + 1, prev.knoisytarget_t + 1, &prev, lexNode, noiselmstate);
              }
            }
          } else{

            baseScore = prev.score + emissions_p[t*N+i] + transitions_v[i*N+prev.tokNet];
            baseScore_forsort = prev.score_forsort + emissions_forsort_p[t*N+i] + transitions_v[i*N+prev.tokNet];
          

            nb_remaining_target = kS - (prev.noisytarget_t + 1);
            nb_remaining_frame = T - (t + 1);
            //if (b==3){
            //  std::cout << "remain target: " << nb_remaining_target << std::endl;
            //  std::cout << "remain frame: " << nb_remaining_frame << std::endl;
            //  std::cout << "prev.knoisytarget_t " << prev.knoisytarget_t << std::endl;
            //  std::cout << "kS " << kS << std::endl;
            //  std::cout << "prev tok " << prev.tokNet << " i: " << i << std::endl;
            //}
            if (prev.tokNet == i) {
              // we force to follow the target if we don't have enough frames to finish it. Except for 1 letter if we allow insertion.
              //std::cout << "nb_remaining_frame " << nb_remaining_frame << " nb_remaining_target " << nb_remaining_target << std::endl;
              ...
              if (nb_remaining_frame >= nb_remaining_target){ //THE PROBLEM IS HERE
                if (nb_remaining_frame - 1 >= nb_remaining_target
                  || (std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t], tok))) // If the next letter can be swapped
                  || (std::isfinite(noiselm_.scoreDeletion(tok)) && (nb_remaining_frame - 1 >= nb_remaining_target)) //if the next letter can be deleted
                  || (std::isfinite(noiselm_.scoreInsertion(knoisytarget_p[prev.knoisytarget_t])) && (prev.knoisytarget_t + 2 < kS) // if the next letter can be inserted
                        && std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 1], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                     )
                  )
                {
                  ...
                  //std::cout << "add hyp: " << std::endl;
                  newhyps.emplace_back(i, prev.tok, baseScore, baseScore_forsort, 0, prev.noisytarget_t, prev.knoisytarget_t, &prev, prev.lexNode, noiselmstate);
              ...
              if (noiselm_.allowInsertion() && prev.knoisytarget_t + 2 < kS){ //a letter has been added to the noisy transcription. +1 only because we can 
                if (std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 2], tok)) // If the next letter can be swapped
                  || (std::isfinite(noiselm_.scoreDeletion(tok)) && (nb_remaining_frame - 1 >= nb_remaining_target - 2)) //if the next letter can be deleted
                  || (std::isfinite(noiselm_.scoreInsertion(knoisytarget_p[prev.knoisytarget_t + 2])) && (prev.knoisytarget_t + 4 < kS) // if the next letter can be inserted
                        && std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 3], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                     )
                  )
                {
                  logProbNoise = noiselm_.scoreInsertion(knoisytarget_p[prev.knoisytarget_t]) + noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 1], prev.tok);
                  noisescore = noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score();
                  ...
                  if (std::isfinite(noisescore)){
                    ...
                    score_forsort = count_noise_sort_ ? baseScore_forsort + noisescore : baseScore_forsort;
                    newhyps.emplace_back(i, tok, score, score_forsort, logProbNoise, prev.noisytarget_t + 2, prev.knoisytarget_t + 2, &prev, lexNode, noiselmstate);
                  }
                }
              }

              base_logProbNoise = noiselm_.allowInsertion() ? noiselm_.scoreNoInsertion() : 0.; //no letter is inserted
              if (noiselm_.allowDeletion() && prev.knoisytarget_t < kS){ // for now allow only 1 deletion and one last deletion
                ...
                if (std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t], tok)) // If the next letter can be swapped
                    || (std::isfinite(noiselm_.scoreDeletion(tok)) && (nb_remaining_frame - 1 >= nb_remaining_target)) //if the next letter can be deleted
                    || (std::isfinite(noiselm_.scoreInsertion(knoisytarget_p[prev.knoisytarget_t])) && (prev.knoisytarget_t + 2 < kS) // if the next letter can be inserted
                        && std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 1], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                       )
                   ) 
                {
                  ...
                  if (nb_remaining_frame >= nb_remaining_target){
                    logProbNoise = base_logProbNoise + noiselm_.scoreDeletion(prev.tok);
                    noisescore = noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score();           
                    ...
                    if (std::isfinite(noisescore)){
                      score = count_noise_ ? baseScore + noisescore : baseScore;
                      score_forsort = count_noise_sort_ ? baseScore_forsort + noisescore : baseScore_forsort;
                      newhyps.emplace_back(i, tok, score, score_forsort, logProbNoise, prev.noisytarget_t, prev.knoisytarget_t, &prev, lexNode, noiselmstate);
                    }
                  }
                }
              }
              ...
              if (noiselm_.allowSwap() && prev.knoisytarget_t + 1 < kS){
                ...
                if (std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 1], tok)) // If the next letter can be swapped
                    || (std::isfinite(noiselm_.scoreDeletion(tok)) && (nb_remaining_frame - 1 >= nb_remaining_target - 1)) //if the next letter can be deleted
                    || (std::isfinite(noiselm_.scoreInsertion(knoisytarget_p[prev.knoisytarget_t + 1])) && (prev.knoisytarget_t + 3 < kS) // if the next letter can be inserted
                        && std::isfinite(noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 2], tok)) // and then the next one can be swapped or deleted (useless to verify since already done)
                       )
                    )
                {
                  logProbNoise = base_logProbNoise + noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t], prev.tok);
                  noisescore = noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score();
                  //std::cout << "noisescore " << knoisytarget_p[prev.knoisytarget_t] << "->" << prev.letter->idx() << " " << noisescore << std::endl;
                  ...
                  if (std::isfinite(noisescore)){
                    score = count_noise_ ? baseScore + noisescore : baseScore;
                    score_forsort = count_noise_sort_ ? baseScore_forsort + noisescore : baseScore_forsort;
                    newhyps.emplace_back(i, tok, score, score_forsort, logProbNoise, prev.noisytarget_t + 1, prev.knoisytarget_t + 1, &prev, lexNode, noiselmstate);
                  }
                }
              }  
            }
    ...
    if(newhyps.size() == 0) {
      ...
      std::sort(newhyps.begin(), newhyps.end(), [](ForceAlignBeamNoiseNode& a, ForceAlignBeamNoiseNode& b) {
          if (a.lexNode == b.lexNode){
            if(a.tokNet == b.tokNet) { // same as a.lex == b.lex but count the rep1 token
      ...
      for(long h = 1; h < nhyp; h++) {
        ...
        if((head.lexNode == elem.lexNode) && (head.tokNet == elem.tokNet) && (head.knoisytarget_t == elem.knoisytarget_t)) { //maybe to change when key level
          if(head.merge_a < 0) {
      ...
      nhyp = std::min(headidx+1, nhyp); // should be headidx+1 when it is working. To handle the case nhyp = 0
      //if (b==3){
      //  std::cout << "nhyp after " << nhyp << std::endl;
      //}
      //
      // beam it
      std::sort(newhyps.begin(), newhyps.begin()+nhyp, [](ForceAlignBeamNoiseNode& a, ForceAlignBeamNoiseNode& b) { return a.score_forsort > b.score_forsort; });
      //std::sort(newhyps.begin(), newhyps.begin()+nhyp, [](ForceAlignBeamNoiseNode& a, ForceAlignBeamNoiseNode& b) { return a.score > b.score; });
    ...
    for(ForceAlignBeamNoiseNode &prev : hyps.at(T)) {
      ...
      if (prev.knoisytarget_t + 1 == kS && noiselm_.allowSwap()){ // no insertion + swap current + no insertion after
        base_logProbNoise = noiselm_.allowInsertion() ? 2.0 * noiselm_.scoreNoInsertion() : 0.;
        logProbNoise = base_logProbNoise + noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t], prev.tok);
        noisescore = noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score();
        merge_it = true;
      } else if (prev.knoisytarget_t + 2 == kS && noiselm_.allowInsertion()){ // insertion + swap current + no insertion
        base_logProbNoise = noiselm_.scoreNoInsertion() + noiselm_.scoreInsertion(knoisytarget_p[prev.knoisytarget_t]);
        logProbNoise = noiselm_.allowSwap() ? base_logProbNoise + noiselm_.scoreSwap(knoisytarget_p[prev.knoisytarget_t + 1], prev.tok) : base_logProbNoise;
        noisescore = noiselm_.allowSwap() ? noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score() : noiselm_.scale_noise() * logProbNoise;
        merge_it = true;
      } else if (prev.knoisytarget_t == kS && noiselm_.allowDeletion()){ //deletion of the previous letter + no insertion after
        logProbNoise = noiselm_.allowInsertion() ? noiselm_.scale_noise() * noiselm_.scoreNoInsertion() : 0.;
        noisescore = noiselm_.scale_noise() * logProbNoise + noiselm_.tkn_score();
        merge_it = true;
      }
      if (merge_it) {
        ...
        score_forsort = count_noise_sort_ ? prev.score_forsort + noisescore : prev.score_forsort;
  ...
  return fl::Variable(result, inputs, grad_func, std::make_shared<ForceAlignBeamNoiseVariablePayload>(data));
}
...
//This is not working
/*
fl::Variable ForceAlignBeamNoise::transformOutput(fl::Variable& beam_transform, fl::Variable& emissions_previous, fl::Variable& emissions_new, fl::Variable& transitions, fl::Variable& noisytarget, fl::Variable& knoisytarget)
{
...
#pragma omp parallel for num_threads(B)
  for(int b = 0; b < B; b++) {
    ...
    for(long t = 1; t <= T; t++) {
      //std::cout << "t " << t << " max old " << hyps.at(t)[0].maxscore << std::endl;
      double maxscore_new = -std::numeric_limits<double>::infinity();
      for(ForceAlignBeamNoiseNode& node : hyps.at(t)) {
        //if (t== 5 || t==6){
        //  std::cout << "before old " << node.score_bis << " new " << node.score << std::endl;
          //std::cout << emissions_previous_p[(t-1)*N+node.l] << " " << emissions_new_p[(t-1)*N+node.l] << std::endl;
        //}
        //
        ...
        if(node.merge_a >= 0) {
          ...
          //}
            ...
            for(long idx = 0; idx < n; idx++) {
              merged_score[idx + node.merge_a] += node.maxscore 
                                                  - (merged[idx + node.merge_a]->score_bis + emissions_previous_p[(t-1)*N+node.l]) //remove old prev score and emission
                                                  + merged[idx + node.merge_a]->score + emissions_new_p[(t-1)*N+node.l];
              maxscore_new = std::max(maxscore_new, merged_score[idx + node.merge_a]);
              //std::cout << "idx = " << idx << std::endl;
              //std::cout << merged[idx + node.merge_a]->score_bis << " " << merged[idx + node.merge_a]->score << std::endl;
              //if (t== 5 || t==6){
              //  std::cout << "idx = " << idx << std::endl;
              //  std::cout << merged[idx + node.merge_a]->score_bis << " " << merged[idx + node.merge_a]->score << std::endl;
              //}
          ...
          node.score += node.maxscore
                      - (node.parent->score_bis + emissions_previous_p[(t-1)*N+node.l])
                      + node.parent->score + emissions_new_p[(t-1)*N+node.l];
          
          maxscore_new = std::max(maxscore_new, node.score);

          //std::cout << node.parent->score_bis << " " << node.parent->score << std::endl;
          //std::cout << emissions_previous_p[(t-1)*N+node.l] << " " << emissions_new_p[(t-1)*N+node.l] << std::endl;
          maxscore_new = std::max(maxscore_new, node.score);
        }
      }
      ...
      std::sort(hyps.at(t).begin(), hyps.at(t).end(), [](ForceAlignBeamNoiseNode& a, ForceAlignBeamNoiseNode& b) { return a.score > b.score; });
    }
    ...
    if(final_node.merge_a >= 0) { //always true
      ...
      for(long idx = 0; idx < n; idx++) {
        merged_score[idx + final_node.merge_a] = merged_score[idx + final_node.merge_a] 
                                            - merged[idx + final_node.merge_a]->score_bis
                                            + merged[idx + final_node.merge_a]->score;
        score_final = logadd(score_final, merged_score[idx + final_node.merge_a]);
      }
    }
    ...
    //std::cout << "final old " << final_node.score_bis << "final new " << final_node.score << std::endl;
  }
  ...
  std::vector<fl::Variable> inputs = {emissions_new, transitions, noisytarget, knoisytarget};
  const std::vector<fl::Variable> noiselmParams = noiselm_.params();
  inputs.insert(inputs.end(), noiselmParams.begin(), noiselmParams.end());
...
static void accnode(ForceAlignBeamNoiseNode& node, ForceAlignBeamNoiseNode& prev, float* gemissions_p, float* gtransitions_p, float* gscale_noise_p, long t, long T, long N, NoiseLMLetterSwapUnit& noiselm, double g)
{
  if(t >= 0 && t < T) {
...
static void connect_nodes(std::ostream& f, long t1, ForceAlignBeamNoiseNode& node1, long t2, ForceAlignBeamNoiseNode& node2)
{
...
//}
/*
void ForceAlignBeamNoise::showTarget(const fl::Variable& output, int64_t b, std::ostream& f)
{
  auto payload = output.getPayload();
  if(!payload) {
    throw std::invalid_argument("expecting a payload on provided Variable");
...
struct pathsInfo {
  ...
  void addPathValue(std::vector<int> path, double value) {
    ...
    if (it_existing_path == pathsToValue.end()) { //if path is not present, we may add it
      //if (value >= threshold){
        if (pathsToValue.size() < max_nb_paths){ // if we have enough space to add a new path
          auto it = pathsToValue.insert({path, value}).first; // create a new path and add value
          ...
          //                [](decltype(pathsToValue)::value_type& l, decltype(pathsToValue)::value_type& r) -> bool { return l.second < r.second; });
          ...
          if (it_min->first < value){ // if the lowest one is worse than the new added value, we remove it and add the new.
            pathsToValue.erase(it_min->second); // erase the corresponding iterator in pathsToValue
...
/*
std::map< std::vector<int>, double> ForceAlignBeamNoise::extractPathsAndWeights(const fl::Variable& output, int64_t b)
{
  auto payload = output.getPayload();
  if(!payload) {
    throw std::invalid_argument("expecting a payload on provided Variable");
  ...
  //sub_merged_score.resize(n);
  //std::copy(merged_score.begin()+fini.merge_a, merged_score.begin()+fini.merge_b+1, sub_merged_score.begin());

  //sub_merged.resize(n);
  //std::copy(merged.begin()+fini.merge_a, merged.begin()+fini.merge_b+1, sub_merged.begin());
  ...
  for(long idx = 0; idx < n; idx++) {
    ...
    pathsInfo_current.addPathValue(new_path, merged_score[idx + fini.merge_a] - node_current->score); //should be equal to the noise added at the end
    ...
    for(ForceAlignBeamNoiseNode& node : hyps.at(t+1)) {
      if(node.active) {
        ...
        if(node.merge_a >= 0) {
          ...
          //sub_merged.resize(n);
          //std::copy(merged.begin()+node.merge_a, merged.begin()+node.merge_b+1, sub_merged.begin());
          ...
          for(long idx = 0; idx < n; idx++) {
            ...
            if(node_score_previous != -std::numeric_limits<double>::infinity()) {
              ...
              for (auto const& x : pathsInfo_previous.pathsToValue){
                ...
                pathsInfo_current.addPathValue(new_path, x.second + node_score_previous + node.maxscore
                                                                - node_current->score);
              }
              //nodeToPaths[(long)(node_current)] = pathsInfo_current;
            }
...
void ForceAlignBeamNoise::backward(std::vector<fl::Variable>& inputs, const fl::Variable& goutput, std::shared_ptr<ForceAlignBeamNoiseBatchData> data)
{
...
#pragma omp parallel for num_threads(B)
      ...
      for(ForceAlignBeamNoiseNode& node : hyps.at(t+1)) {
        if(node.active) {
          if(node.merge_a >= 0) {
            ...
            std::copy(merged_score.begin()+node.merge_a, merged_score.begin()+node.merge_b+1, sub_merged_score.begin()); // could remove copy
            dlogadd(sub_merged_score, sub_merged_gscore, node.gscore);
            for(long idx = 0; idx < n; idx++) {
              if(sub_merged_score[idx] != -std::numeric_limits<double>::infinity()) {
                accnode(node, *merged.at(node.merge_a+idx), gemissions_p, gtransitions_p, gscale_noise_p, t, T, N, noiselm_, sub_merged_gscore[idx]);
              }
            }
          } else {
            accnode(node, *node.parent, gemissions_p, gtransitions_p, gscale_noise_p, t, T, N, noiselm_, node.gscore);
          }
        }
      }
    }
  ...
  //noiselmParam.addGrad(fl::Variable(af::array(1, gscale_noise_v.data()), false));
}
...
struct pathsInfoBeamBackward {
  typedef std::tuple<std::vector<int>, double, std::map<ForceAlignBeamNoiseNode*, double>, double> pathTuple;
  ...
  void addPathValue(std::vector<int> path, double value, ForceAlignBeamNoiseNode* node_ptr) {
    ...
    if (it_existing_path == pathsInfo.end()) { //if the path is not present, we add it
      std::map<ForceAlignBeamNoiseNode*, double> single_map = {{node_ptr, value}};
...
std::vector< std::tuple<std::vector<int>, double>> ForceAlignBeamNoise::extractPathsAndWeightsBackward(const fl::Variable& output, int b, int beam_size)
{
  auto payload = output.getPayload();
  if(!payload) {
    throw std::invalid_argument("expecting a payload on provided Variable");
  }
  auto& data = std::dynamic_pointer_cast<ForceAlignBeamNoiseVariablePayload>(payload)->data->batch[b];
      ...
      for (auto const& prev_node_info : prev_nodes){
        ...
        if(prev_node->merge_a >= 0) {
          ...
          for(long idx = 0; idx < n; idx++) {
            ...
            if (new_path.size() == 0 || (new_path.back() != current_node->tokNet && t != 0)){
...
double ForceAlignBeamNoise::getTrueScore(const fl::Variable& output, int b, std::vector<int> pathAnalysis)
{
  auto payload = output.getPayload();
  if(!payload) {
    throw std::invalid_argument("expecting a payload on provided Variable");
...
std::tuple<double, std::vector<std::vector< std::tuple<std::vector<int>, double>>> > ForceAlignBeamNoise::wLER(const fl::Variable& output, fl::Variable& cleantarget, int beam_size, fl::AverageValueMeter* mtr_wLER)
{
  auto payload = output.getPayload();
  if(!payload) {
    throw std::invalid_argument("expecting a payload on provided Variable");
  }
  auto& data = std::dynamic_pointer_cast<ForceAlignBeamNoiseVariablePayload>(payload)->data->batch;

  int B = data.size();
  std::vector<double> all_wLER(B,0);
  std::vector<std::vector< std::tuple<std::vector<int>, double>>> all_paths_weights(B);
#pragma omp parallel for num_threads(B)
  for(int b = 0; b < B; b++) {
  ...
  for(int b = 0; b < B; b++) {
    ...
    for ( const auto &p_v : paths_weights ) {
      ...
      mtr_LER.add(path.data(), tgtraw_clean.data(), path.size(), tgtraw_clean.size());
