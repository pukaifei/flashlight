...
//removed LM& lm,
AutoSegBeamNoiseCriterion::AutoSegBeamNoiseCriterion(int N, w2l::Dictionary& tokenDict, std::shared_ptr<NoiseTrie> lex, NoiseLMLetterSwapUnit& noiselm, long beamsize, w2l::CriterionScaleMode scaleMode, double threshold, bool computeStats, int top_k, bool useevalemission,  bool useNoiseToSort)
  : fal_(tokenDict, lex, noiselm, beamsize, threshold, top_k, true, useNoiseToSort), falStats_(tokenDict, lex, noiselm, beamsize, threshold), fcc_(N, scaleMode), scaleMode_(scaleMode), N_(N), fal_true_(N, scaleMode), computeStats_(computeStats), useevalemission_(useevalemission)
{
...
std::vector<fl::Variable> AutoSegBeamNoiseCriterion::forward(fl::Variable& emissions, fl::Variable& emissions_eval, fl::Variable transitions, fl::Variable target, fl::Variable ktarget, fl::Variable cleantarget, int statsbeamsize, fl::AverageValueMeter* mtr_wLER)
{
  ...
  */
  ...
  w2l::cpu::CriterionUtils<float>::computeScale(B, T, N, scaleMode_, targetSizeVec.data(), scaleVec.data());
  ...
  if (useevalemission_){
    ...
    fal_output_beam = fal_.forward(emissions, emissions_eval, transitions, target, ktarget);
  } else{
  ...
  //auto fal_output_beam = fal_.forward(emissions, transitions, target, ktarget);
    //af::print("fal_output_beam emissions_train", fal_output_beam.array());

  if (computeStats_){
    auto wLER_paths = fal_.wLER(fal_output_beam, cleantarget, statsbeamsize, mtr_wLER);
  }
  ...
  if (af::anyTrue<bool>(nanValues)) { // C'est du bricolage, ça n'arrive quasiment jamais
