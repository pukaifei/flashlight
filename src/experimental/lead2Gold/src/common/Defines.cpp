...
DEFINE_string(
    noisetarget,
    "noisesplit",
    "noise target split. Separate the sentence into noisy keys");
DEFINE_string(
    cleantarget,
    "",
    "clean target split. Separate the sentence into clean keys");
...
DEFINE_bool(
    storeGroundTruth,
    false,
    "if true the second tab separated string in a FileListDataset is the ground truth");
DEFINE_bool(
    storeForNoiselm,
    false,
    "if true kCleanNoiselmKeyIdx and kNoisyNoiselmKeyIdx fields are used");
...
DEFINE_double(
    scale_noise,
    0.4,
    "multiplicative scaling to log(p noise) if autoScale is false for substitutions");
DEFINE_double(
    scale_sub,
    1,
    "additive scaling to log(p noise) if autoScale is false for substitutions");
DEFINE_double(
    scale_ins,
    1,
    "additive scaling to log(p noise) if autoScale is false for insertions");
DEFINE_double(
    scale_del,
    1,
    "additive scaling to log(p noise) if autoScale is false for deletions");
DEFINE_double(
    tkn_score,
    0,
    "additive scaling to log(p noise) if autoScale is false for deletions");
DEFINE_bool(
    computeStats,
    false,
    "compute stats for each node, make the code slow");
DEFINE_int64(
    topk,
    0,
    "Take top k tokens from emission scores to expand hypotheses. No pruning if topk=0");
DEFINE_bool(
    useevalemission,
    false,
    "compute emission with evaluation mode to get a better wLER accuracy");
DEFINE_bool(useNoiseToSort, true, "use noisescore to sort hypotheses");

DEFINE_string(
    saveExamplePathFolder,
    "",
    "path were to cerialize emission probas");
DEFINE_string(
    computeStatsStorePath,
    "",
    "path were to store results in a json format");
DEFINE_bool(
    computeStatsLight,
    false,
    "if true do not store the stats for every frame");
DEFINE_int64(
    nb_ex,
    20,
    "Number of maximum example to process for computing stats");

DEFINE_string(statsDirection, "forward", "direction to retrieve paths");
DEFINE_int64(
    statsbeamsize,
    10,
    "size of the beam when searching for the paths in stats mode");

DEFINE_int64(
    nbNested,
    1,
    "Nested loop parallel mode. Nb of threads to span for each example. Make sure batchsize * nbNested <= available core ");
...
DEFINE_int64(
    saveevery,
    1,
    "if itersave is true, save model every saveevery iteration");
// DEFINE_int64(warmup, 8000, "the LR warmup parameter, in batches");
// DEFINE_bool(use_saug, false, "Use SpecAugment");
// DEFINE_int64(lr_decay, std::numeric_limits<int64_t>::max(), "Epoch for the
// first LR decay"); DEFINE_int64(lr_decay_step,
// std::numeric_limits<int64_t>::max(), "Epochs for each new LR decay");
...
DEFINE_bool(
    evalbeamnoise,
    false,
    "if true compute asgbeamnoise criterion loss on dev datasets");
DEFINE_bool(evalnoiselm, false, "if true eval noiselm");
DEFINE_bool(
    updateOnTheFly,
    false,
    "if true update transcriptions every time we need them");
...
DEFINE_bool(
    useSinPosEmb,
    false,
    "use sinus positional embedding instead of learning it");
DEFINE_bool(
    usePosEmbEveryLayer,
    false,
    "use positional embedding at every layer");
...
/* ========== RNN noiselm ========== */
DEFINE_int64(encoderrnnlayer, 1, "Number of RNN layers for the encoder");

DEFINE_double(encoderdropout, 0, "dropout for the encoder");
