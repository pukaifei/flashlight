...
int main(int argc, char** argv) {
  ...
  // auto noiselmoptim = initOptimizer({noiselm}, FLAGS_noiselmoptim,
  // FLAGS_lrnoiselm, 0.0, 0.0);
  auto noiselmoptim = std::make_shared<fl::SGDOptimizer>(
      noiselm->params(), FLAGS_lrscalenoise, 0.0, 0.0);
  ...
  for (auto& sample : *ds) {
    ...
    w2l::cpu::CriterionUtils<float>::computeScale(
        B, T, N_, scalemode, targetSizeVec.data(), scaleVec.data());
    fl::Variable scale(af::array(B, scaleVec.data()), false);

    fac_beam_output_noisy =
        scale * fac_beam.forward(emission_train, transition, target, ktarget);
