...
def process_experiment(dir, args):
    ...
    writer = Writer(os.path.join(args.outdir, basename))
    perfid = 1
    epoch = 1
    while True:
        ...
        for idx in range(1, len(content)):
            ...
            times = []
            times.extend(int(v) for v in vals[0].split("-"))
            times.extend(int(v) for v in vals[1].split(":"))
            times.extend([0, 0, 0])

            writer.write(summary, epoch, time.mktime(tuple(times)))
            epoch = epoch + 1
        perfid = perfid + 1
    writer.close()
